...
const nsForwardReference::Phase nsForwardReference::kPasses[] = {
    nsForwardReference::eConstruction, nsForwardReference::eHookup,
...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN_INHERITED(XULDocument, XMLDocument)
  ...
  if (tmp->mOverlayLoadObservers) {
    for (auto iter = tmp->mOverlayLoadObservers->Iter(); !iter.Done();
         iter.Next()) {
  ...
  if (tmp->mPendingOverlayLoadNotifications) {
    for (auto iter = tmp->mPendingOverlayLoadNotifications->Iter();
      ...
      NS_CYCLE_COLLECTION_NOTE_EDGE_NAME(
          cb, "mPendingOverlayLoadNotifications value");
      cb.NoteXPCOMChild(iter.Data());
    }
  }
...
bool XULDocument::OnDocumentParserError() {
...
void XULDocument::SynchronizeBroadcastListener(Element* aBroadcaster,
                                               Element* aListener,
...
void XULDocument::AddBroadcastListenerFor(Element& aBroadcaster,
                                          Element& aListener,
                                          const nsAString& aAttr,
                                          ErrorResult& aRv) {
...
void XULDocument::RemoveBroadcastListenerFor(Element& aBroadcaster,
                                             Element& aListener,
                                             const nsAString& aAttr) {
...
nsresult XULDocument::ExecuteOnBroadcastHandlerFor(Element* aBroadcaster,
                                                   Element* aListener,
                                                   nsAtom* aAttr) {
...
void XULDocument::AttributeChanged(Element* aElement, int32_t aNameSpaceID,
  ...
  if (ShouldPersistAttribute(aElement, aAttribute) && !persist.IsEmpty() &&
    ...
    nsContentUtils::AddScriptRunner(
        NewRunnableMethod<Element*, int32_t, nsAtom*>(
            "dom::XULDocument::DoPersist", this, &XULDocument::DoPersist,
...
nsresult XULDocument::AddForwardReference(nsForwardReference* aRef) {
...
nsresult XULDocument::ResolveForwardReferences() {
  if (mResolutionPhase == nsForwardReference::eDone) return NS_OK;
...
already_AddRefed<nsINodeList> XULDocument::GetElementsByAttribute(
...
already_AddRefed<nsINodeList> XULDocument::GetElementsByAttributeNS(
...
void XULDocument::Persist(const nsAString& aID, const nsAString& aAttr,
                          ErrorResult& aRv) {
  ...
  RefPtr<mozilla::dom::NodeInfo> ni =
      element->GetExistingAttrNameFromQName(aAttr);
  nsresult rv;
  if (ni) {
    tag = ni->NameAtom();
...
nsresult XULDocument::Persist(Element* aElement, int32_t aNameSpaceID,
                              nsAtom* aAttribute) {
...
nsresult XULDocument::GetViewportSize(int32_t* aWidth, int32_t* aHeight) {
  *aWidth = *aHeight = 0;
...
int32_t XULDocument::GetWidth(ErrorResult& aRv) {
  int32_t width = 0;
  int32_t height = 0;
...
int32_t XULDocument::GetHeight(ErrorResult& aRv) {
  int32_t width = 0;
  int32_t height = 0;
  aRv = GetViewportSize(&width, &height);
  return height;
...
already_AddRefed<nsINode> XULDocument::GetPopupNode() {
...
void XULDocument::SetPopupNode(nsINode* aNode) {
  nsCOMPtr<nsPIWindowRoot> rootWin = GetWindowRoot();
  if (rootWin) {
...
// chrome callers only.
nsINode* XULDocument::GetPopupRangeParent(ErrorResult& aRv) {
  nsXULPopupManager* pm = nsXULPopupManager::GetInstance();
  if (!pm) {
...
int32_t XULDocument::GetPopupRangeOffset(ErrorResult& aRv) {
  nsXULPopupManager* pm = nsXULPopupManager::GetInstance();
  if (!pm) {
    aRv.Throw(NS_ERROR_FAILURE);
    return 0;
...
already_AddRefed<nsINode> XULDocument::GetTooltipNode() {
  nsXULPopupManager* pm = nsXULPopupManager::GetInstance();
  if (pm) {
...
nsresult XULDocument::AddElementToDocumentPre(Element* aElement) {
  ...
  if (listener && !resolved &&
      (mResolutionPhase != nsForwardReference::eDone)) {
...
nsresult XULDocument::AddElementToDocumentPost(Element* aElement) {
...
nsresult XULDocument::AddSubtreeToDocument(nsIContent* aContent) {
...
nsresult XULDocument::RemoveSubtreeFromDocument(nsIContent* aContent) {
  // From here on we only care about elements.
  if (!aContent->IsElement()) {
    return NS_OK;
  }
...
/* static */
bool XULDocument::MatchAttribute(Element* aElement, int32_t aNamespaceID,
...
nsresult XULDocument::ApplyPersistentAttributes() {
  // For non-chrome documents, persistance is simply broken
  if (!nsContentUtils::IsSystemPrincipal(NodePrincipal()))
    return NS_ERROR_NOT_AVAILABLE;
...
nsresult XULDocument::ApplyPersistentAttributesInternal() {
...
nsresult XULDocument::ApplyPersistentAttributesToElements(
...
nsresult XULDocument::InsertXULOverlayPI(const nsXULPrototypePI* aProtoPI,
  ...
  nsAutoString href;
  nsContentUtils::GetPseudoAttributeValue(aProtoPI->mData, nsGkAtoms::href,
                                          href);
...
nsresult XULDocument::AddChromeOverlays() {
  nsresult rv;
...
void XULDocument::LoadOverlay(const nsAString& aURL, nsIObserver* aObserver,
                              ErrorResult& aRv) {
  nsresult rv;
...
nsresult XULDocument::LoadOverlayInternal(nsIURI* aURI, bool aIsDynamic,
                                          bool* aShouldReturn,
  ...
  *aFailureFromContent = false;
  ...
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(
        ...
        ("xul: %s loading overlay %s", uri ? uri->GetSpecOrDefault().get() : "",
         aURI->GetSpecOrDefault().get()));
  }

  if (aIsDynamic) mResolutionPhase = nsForwardReference::eStart;
  ...
  bool documentIsChrome = mDocumentURI ? IsChromeURI(mDocumentURI) : false;
  mCurrentPrototype =
      overlayIsChrome && documentIsChrome
          ? nsXULPrototypeCache::GetInstance()->GetPrototype(aURI)
          : nullptr;
  ...
  if (useXULCache && mCurrentPrototype) {
    ...
    if (!listener) return NS_ERROR_UNEXPECTED;
    ...
    // compartment.
    rv = NS_NewChannel(getter_AddRefs(channel), aURI, NodePrincipal(),
    ...
    if (!aIsDynamic) *aShouldReturn = true;
  }
  return NS_OK;
}
...
nsresult XULDocument::ResumeWalk() {
  ...
  while (1) {
    ...
    while (mContextStack.Depth() > 0) {
        ...
        if (element) {
          ...
          if (mState == eState_Master) {
            ...
            if (element->NodeInfo()->Equals(nsGkAtoms::style,
              ...
              bool willNotify;
              bool isAlternate;
              ssle->UpdateStyleSheet(nullptr, &willNotify, &isAlternate);
            }
          }
        }
      ...
      bool processingOverlayHookupNodes =
          (mState == eState_Overlay) && (mContextStack.Depth() == 1);
      ...
      switch (childproto->mType) {
        case nsXULPrototypeNode::eType_Element: {
          ...
          if (!processingOverlayHookupNodes) {
            ...
            if (mState == eState_Master) AddElementToDocumentPre(child);
          } else {
        ...
        case nsXULPrototypeNode::eType_PI: {
          ...
          if (piProto->mTarget.EqualsLiteral("xml-stylesheet") ||
            ...
            nsContentUtils::ReportToConsole(
                ...
                params, ArrayLength(params), overlayURI);
          }

          nsIContent* parent =
              processingOverlayHookupNodes ? GetRootElement() : element.get();
    ...
    bool shouldReturn, failureFromContent;
    rv = LoadOverlayInternal(uri, false, &shouldReturn, &failureFromContent);
...
nsresult XULDocument::DoneWalking() {
  ...
  if (!mDocumentLoaded) {
    ...
    nsContentUtils::DispatchTrustedEvent(
        ...
        NS_LITERAL_STRING("MozBeforeInitialXULLayout"), true, false);
    ...
    if (mPendingOverlayLoadNotifications) {
      ...
      for (auto iter = mPendingOverlayLoadNotifications->Iter(); !iter.Done();
           iter.Next()) {
        nsIURI* aKey = iter.Key();
        iter.Data()->Observe(aKey, "xul-overlay-merged", EmptyString().get());
...
NS_IMETHODIMP
XULDocument::StyleSheetLoaded(StyleSheet* aSheet, bool aWasAlternate,
                              nsresult aStatus) {
  if (!aWasAlternate) {
...
void XULDocument::MaybeBroadcast() {
  // Only broadcast when not in an update and when safe to run scripts.
  if (mUpdateNestLevel == 0 && (mDelayedAttrChangeBroadcasts.Length() ||
                                mDelayedBroadcasters.Length())) {
    if (!nsContentUtils::IsSafeToRunScript()) {
      if (!mInDestructor) {
        nsContentUtils::AddScriptRunner(
            NewRunnableMethod("dom::XULDocument::MaybeBroadcast", this,
...
void XULDocument::EndUpdate(nsUpdateType aUpdateType) {
  XMLDocument::EndUpdate(aUpdateType);

  MaybeBroadcast();
}

void XULDocument::ReportMissingOverlay(nsIURI* aURI) {
  NS_PRECONDITION(aURI, "Must have a URI");

  NS_ConvertUTF8toUTF16 utfSpec(aURI->GetSpecOrDefault());
  const char16_t* params[] = {utfSpec.get()};
  nsContentUtils::ReportToConsole(
      nsIScriptError::warningFlag, NS_LITERAL_CSTRING("XUL Document"), this,
      nsContentUtils::eXUL_PROPERTIES, "MissingOverlay", params,
      ArrayLength(params));
}
...
nsresult XULDocument::ExecuteScript(nsXULPrototypeScript* aScript) {
  ...
  JSAddonId* addonId = mCurrentPrototype
                           ? MapURIToAddonID(mCurrentPrototype->GetURI())
                           : nullptr;
...
nsresult XULDocument::CreateElementFromPrototype(
  ...
  *aResult = nullptr;
  ...
  if (aPrototype->mNodeInfo->NamespaceEquals(kNameSpaceID_XUL)) {
    ...
    // monkeys with it.
    rv = nsXULElement::Create(aPrototype, this, true, aIsRoot,
...
nsresult XULDocument::CreateOverlayElement(nsXULPrototypeElement* aPrototype,
                                           Element** aResult) {
  nsresult rv;
...
nsresult XULDocument::AddPrototypeSheets() {
  nsresult rv;

  const nsCOMArray<nsIURI>& sheets =
      mCurrentPrototype->GetStyleSheetReferences();

  for (int32_t i = 0; i < sheets.Count(); i++) {
    nsCOMPtr<nsIURI> uri = sheets[i];

    RefPtr<StyleSheet> incompleteSheet;
    rv = CSSLoader()->LoadSheet(uri, mCurrentPrototype->DocumentPrincipal(),
                                EmptyCString(), this, &incompleteSheet);
...
nsForwardReference::Result XULDocument::OverlayForwardReference::Resolve() {
  ...
  if (id.IsEmpty()) {
    ...
    // until later.
    if (!target) return eResolve_Later;
  ...
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(gXULLog, LogLevel::Debug,
            ("xul: overlay resolved '%s'", idC.get()));
  }
...
nsresult XULDocument::OverlayForwardReference::Merge(Element* aTargetElement,
  ...
  for (i = 0; (name = aOverlayElement->GetAttrNameAt(i)); ++i) {
    ...
    if (name->Equals(nsGkAtoms::id)) continue;
...
nsForwardReference::Result XULDocument::BroadcasterHookup::Resolve() {
  nsresult rv;

  bool listener;
  rv = mDocument->CheckBroadcasterHookup(mObservesElement, &listener,
                                         &mResolved);
  if (NS_FAILED(rv)) return eResolve_Error;
...
XULDocument::BroadcasterHookup::~BroadcasterHookup() {
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Warning) && !mResolved) {
    ...
    if (mObservesElement->IsXULElement(nsGkAtoms::observes)) {
      mObservesElement->GetAttr(kNameSpaceID_None, nsGkAtoms::element,
                                broadcasterID);
      mObservesElement->GetAttr(kNameSpaceID_None, nsGkAtoms::attribute,
                                attribute);
    } else {
      mObservesElement->GetAttr(kNameSpaceID_None, nsGkAtoms::observes,
                                broadcasterID);
      attribute.Assign('*');
    }
...
nsresult XULDocument::BroadcastAttributeChangeFromOverlay(
    nsIContent* aNode, int32_t aNameSpaceID, nsAtom* aAttribute,
    nsAtom* aPrefix, const nsAString& aValue) {
  nsresult rv = NS_OK;

  if (!mBroadcasterMap || !CanBroadcast(aNameSpaceID, aAttribute)) return rv;

  if (!aNode->IsElement()) return rv;
  ...
  for (size_t i = entry->mListeners.Length() - 1; i != (size_t)-1; --i) {
    ...
    if (l) {
      rv = l->SetAttr(aNameSpaceID, aAttribute, aPrefix, aValue, false);
      if (NS_FAILED(rv)) return rv;
    }
  }
  return rv;
...
nsresult XULDocument::FindBroadcaster(Element* aElement, Element** aListener,
  ...
  *aBroadcaster = nullptr;
  ...
  if (ni->Equals(nsGkAtoms::observes, kNameSpaceID_XUL)) {
    ...
    if (parent->NodeInfo()->Equals(nsGkAtoms::overlay, kNameSpaceID_XUL)) {
...
nsresult XULDocument::CheckBroadcasterHookup(Element* aElement,
                                             bool* aNeedsHookup,
                                             bool* aDidResolve) {
  ...
  *aDidResolve = false;
  ...
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Debug)) {
    nsCOMPtr<nsIContent> content = do_QueryInterface(listener);

    NS_ASSERTION(content != nullptr, "not an nsIContent");
    if (!content) return rv;
...
nsresult XULDocument::InsertElement(nsINode* aParent, nsIContent* aChild,
                                    bool aNotify) {
  ...
  if (aChild->IsElement()) {
    aChild->AsElement()->GetAttr(kNameSpaceID_None, nsGkAtoms::insertafter,
                                 posStr);
  }
  ...
  if (posStr.IsEmpty()) {
    if (aChild->IsElement()) {
      aChild->AsElement()->GetAttr(kNameSpaceID_None, nsGkAtoms::insertbefore,
                                   posStr);
    }
    isInsertAfter = false;
  }
  ...
  if (!posStr.IsEmpty()) {
    ...
    if (content) {
      if (content->GetParent() == aParent) {
        ...
        nsresult rv =
            aParent->InsertChildBefore(aChild, nodeToInsertBefore, aNotify);
        if (NS_FAILED(rv)) {
          return rv;
        }
  ...
  if (!wasInserted) {
    if (aChild->IsElement() &&
        aChild->AsElement()->GetAttr(kNameSpaceID_None, nsGkAtoms::position,
                                     posStr) &&
        !posStr.IsEmpty()) {
      nsresult rv;
      ...
      if (NS_SUCCEEDED(rv) && pos > 0 &&
        ...
        rv = aParent->InsertChildBefore(aChild, nodeToInsertBefore, aNotify);
        if (NS_SUCCEEDED(rv)) wasInserted = true;
...
already_AddRefed<nsPIWindowRoot> XULDocument::GetWindowRoot() {
...
void XULDocument::DirectionChanged(const char* aPrefName, void* aData) {
...
nsIDocument::DocumentTheme XULDocument::GetDocumentLWTheme() {
...
nsIDocument::DocumentTheme XULDocument::ThreadSafeGetDocumentLWTheme() const {
  if (mDocLWTheme != Doc_Theme_Uninitialized) {
    return mDocLWTheme;
  }
  ...
  if (element &&
      ...
      !(hasLWTheme.IsEmpty()) && hasLWTheme.EqualsLiteral("true")) {
