...
static int32_t FindInt32(const MetaData* mMetaData, uint32_t mKey) {
  int32_t value;
...
static int64_t FindInt64(const MetaData* mMetaData, uint32_t mKey) {
  int64_t value;
...
static bool FindData(const MetaData* aMetaData, uint32_t aKey,
                     mozilla::Vector<T, N>* aDest) {
  const void* data;
  size_t size;
  uint32_t type;
...
static void UpdateTrackInfo(mozilla::TrackInfo& aConfig,
...
void MP4AudioInfo::Update(const MetaData* aMetaData,
                          const char* aMimeType) {
...
void MP4VideoInfo::Update(const MetaData* aMetaData,
                          const char* aMimeType) {
  UpdateTrackInfo(*this, aMetaData, aMimeType);
