...
enum UnsetAction { eUnsetInitial, eUnsetInherit };

}  // namespace mozilla

void* nsConditionalResetStyleData::GetConditionalStyleData(
    nsStyleStructID aSID, GeckoStyleContext* aStyleContext) const {
  Entry* e = static_cast<Entry*>(mEntries[aSID]);
  MOZ_ASSERT(e,
             "if mConditionalBits bit is set, we must have at least one "
...
// will be created.
static already_AddRefed<imgRequestProxy> CreateImageRequest(
    nsPresContext* aPresContext, const nsCSSValue& aValue) {
  RefPtr<imgRequestProxy> req = aValue.GetPossiblyStaticImageValue(
      aPresContext->Document(), aPresContext);
  return req.forget();
}

static already_AddRefed<nsStyleImageRequest> CreateStyleImageRequest(
    nsPresContext* aPresContext, const nsCSSValue& aValue,
    nsStyleImageRequest::Mode aModeFlags = nsStyleImageRequest::Mode::Track) {
  css::ImageValue* imageValue = aValue.GetImageStructValue();
  ImageTracker* imageTracker = (aModeFlags & nsStyleImageRequest::Mode::Track)
...
static void SetStyleShapeSourceToCSSValue(StyleShapeSource* aShapeSource,
 ...
 * computed-style form. */
static void ComputePositionValue(GeckoStyleContext* aStyleContext,
                                 const nsCSSValue& aValue,
...
/* static */ PLDHashNumber nsRuleNode::ChildrenHashHashKey(const void* aKey) {
  const nsRuleNode::Key* key = static_cast<const nsRuleNode::Key*>(aKey);
...
/* static */ bool nsRuleNode::ChildrenHashMatchEntry(
    const PLDHashEntryHdr* aHdr, const void* aKey) {
  const ChildrenHashEntry* entry = static_cast<const ChildrenHashEntry*>(aHdr);
  const nsRuleNode::Key* key = static_cast<const nsRuleNode::Key*>(aKey);
  return entry->mRuleNode->GetKey() == *key;
}

/* static */ const PLDHashTableOps nsRuleNode::ChildrenHashOps = {
    ...
    ChildrenHashHashKey, ChildrenHashMatchEntry, PLDHashTable::MoveEntryStub,
...
/* static */
void nsRuleNode::EnsureBlockDisplay(StyleDisplay& display,
  ...
  switch (display) {
    case StyleDisplay::ListItem:
      if (aConvertListItem) {
        ...
        break;
      }  // else, fall through to share the 'break' for non-changing display
         // vals
      MOZ_FALLTHROUGH;
...
/* static */
void nsRuleNode::EnsureInlineDisplay(StyleDisplay& display) {
...
static nscoord CalcLengthWith(const nsCSSValue& aValue, nscoord aFontSize,
...
struct CalcLengthCalcOps : public css::BasicCoordCalcOps,
  ...
  bool ComputeLeafValue(result_type& aResult, const nsCSSValue& aValue) {
    aResult = CalcLengthWith(aValue, mFontSize, mStyleFont, mStyleContext,
...
static inline nscoord ScaleViewportCoordTrunc(const nsCSSValue& aValue,
  ...
  return NSToCoordTruncClamped(aValue.GetFloatValue() * aViewportSize / 100.0f);
}

/* static */
already_AddRefed<nsFontMetrics> nsRuleNode::GetMetricsFor(
  ...
  return nsLayoutUtils::GetMetricsFor(aPresContext, isVertical, aStyleFont,
...
static nsSize CalcViewportUnitsScale(nsPresContext* aPresContext) {
  ...
  if (scrollFrame) {
    ...
    if (styles.mHorizontal == NS_STYLE_OVERFLOW_SCROLL ||
      ...
      nsMargin sizes(
          scrollFrame->GetDesiredScrollbarSizes(aPresContext, context));
...
static nscoord CalcLengthWith(const nsCSSValue& aValue, nscoord aFontSize,
  ...
  if (aValue.IsCalcUnit()) {
    ...
    CalcLengthCalcOps ops(aFontSize, aStyleFont, aStyleContext, aPresContext,
                          aUseProvidedRootEmSize, aUseUserFontSet, aConditions);
    nscoord result;
  ...
  switch (aValue.GetUnit()) {
    ...
    case eCSSUnit_XHeight: {
      ...
      RefPtr<nsFontMetrics> fm = nsRuleNode::GetMetricsFor(
          aPresContext, aStyleContext, styleFont, aFontSize, aUseUserFontSet);
...
/* static */ nscoord nsRuleNode::CalcLength(
  ...
  return CalcLengthWith(aValue, -1, nullptr, aStyleContext, aPresContext, false,
                        true, aConditions);
}
...
static inline nscoord CalcLength(const nsCSSValue& aValue,
                                 ...
                                 nsPresContext* aPresContext,
                                 RuleNodeCacheConditions& aConditions) {
  return nsRuleNode::CalcLength(aValue, aStyleContext, aPresContext,
                                aConditions);
}

/* static */ nscoord nsRuleNode::CalcLengthWithInitialFont(
    nsPresContext* aPresContext, const nsCSSValue& aValue) {
  nsStyleFont defaultFont(aPresContext);  // FIXME: best language?
  RuleNodeCacheConditions conditions;
  return CalcLengthWith(aValue, -1, &defaultFont, nullptr, aPresContext, true,
                        false, conditions);
}
...
struct LengthPercentPairCalcOps : public css::FloatCoeffsAlreadyNormalizedOps {
  ...
  bool ComputeLeafValue(result_type& aResult, const nsCSSValue& aValue) {
    ...
    aResult = result_type(
        CalcLength(aValue, mContext, mPresContext, mConditions), 0.0f);
    return true;
  }
  ...
  result_type MergeAdditive(nsCSSUnit aCalcFunction, result_type aValue1,
    ...
    if (aCalcFunction == eCSSUnit_Calc_Plus) {
      return result_type(NSCoordSaturatingAdd(aValue1.mLength, aValue2.mLength),
    ...
    return result_type(
        NSCoordSaturatingSubtract(aValue1.mLength, aValue2.mLength, 0),
        aValue1.mPercent - aValue2.mPercent);
  }

  result_type MergeMultiplicativeL(nsCSSUnit aCalcFunction, float aValue1,
                                   result_type aValue2) {
...
static void SpecifiedCalcToComputedCalc(const nsCSSValue& aValue,
                                        nsStyleCoord& aCoord,
                                        nsStyleContext* aStyleContext,
                                        RuleNodeCacheConditions& aConditions) {
...
/* static */ nsRuleNode::ComputedCalc nsRuleNode::SpecifiedCalcToComputedCalc(
    const nsCSSValue& aValue, nsStyleContext* aStyleContext,
    nsPresContext* aPresContext, RuleNodeCacheConditions& aConditions) {
  LengthPercentPairCalcOps ops(aStyleContext, aPresContext, aConditions);
 ...
 */
static float GetFloatFromBoxPosition(int32_t aEnumValue) {
  switch (aEnumValue) {
...
#define SETCOORD_CALC_CLAMP_NONNEGATIVE \
  0x00040000  // modifier for CALC_LENGTH_ONLY
...
static bool SetCoord(const nsCSSValue& aValue, nsStyleCoord& aCoord,
                     const nsStyleCoord& aParentCoord, int32_t aMask,
                     GeckoStyleContext* aStyleContext,
                     nsPresContext* aPresContext,
                     RuleNodeCacheConditions& aConditions) {
  bool result = true;
  if (aValue.GetUnit() == eCSSUnit_Null) {
    result = false;
  } else if ((((aMask & SETCOORD_LENGTH) != 0) && aValue.IsLengthUnit()) ||
             (((aMask & SETCOORD_CALC_LENGTH_ONLY) != 0) &&
              aValue.IsCalcUnit())) {
    nscoord len = CalcLength(aValue, aStyleContext, aPresContext, aConditions);
  ...
  } else if (((aMask & SETCOORD_STORE_CALC) != 0) && (aValue.IsCalcUnit())) {
    SpecifiedCalcToComputedCalc(aValue, aCoord, aStyleContext, aConditions);
  ...
  } else if ((aMask & SETCOORD_ANGLE) != 0 && (aValue.IsAngularUnit())) {
    nsStyleUnit unit;
    switch (aValue.GetUnit()) {
...
static inline bool SetAbsCoord(const nsCSSValue& aValue, nsStyleCoord& aCoord,
                               int32_t aMask) {
  ...
  bool rv = SetCoord(aValue, aCoord, dummyParentCoord, aMask, dummyStyleContext,
                     dummyPresContext, dummyCacheKey);
 ...
 */
static bool SetPairCoords(const nsCSSValue& aValue, nsStyleCoord& aCoordX,
                          nsStyleCoord& aCoordY, const nsStyleCoord& aParentX,
                          const nsStyleCoord& aParentY, int32_t aMask,
                          GeckoStyleContext* aStyleContext,
                          nsPresContext* aPresContext,
                          RuleNodeCacheConditions& aConditions) {
  const nsCSSValue& valX = aValue.GetUnit() == eCSSUnit_Pair
                               ? aValue.GetPairValue().mXValue
                               : aValue;
  const nsCSSValue& valY = aValue.GetUnit() == eCSSUnit_Pair
                               ? aValue.GetPairValue().mYValue
                               : aValue;

  bool cX = SetCoord(valX, aCoordX, aParentX, aMask, aStyleContext,
                     aPresContext, aConditions);
  mozilla::DebugOnly<bool> cY = SetCoord(
      valY, aCoordY, aParentY, aMask, aStyleContext, aPresContext, aConditions);
  ...
  if (aValue.IsNumericColorUnit()) {
    ...
    if (0 <= intValue) {
      ...
      bool useStandinsForNativeColors =
          aPresContext && !aPresContext->IsChrome();
...
static void SetComplexColor(const nsCSSValue& aValue,
  ...
  if (unit == eCSSUnit_Initial ||
    ...
    nscolor resultColor;
    if (!SetColor(aValue, aParentColor.mColor, aPresContext, nullptr,
                  resultColor, aConditions)) {
...
template <UnsetAction UnsetTo>
static Maybe<nscoord> ComputeLineWidthValue(
    const nsCSSValue& aValue, const nscoord aParentCoord,
    const nscoord aInitialCoord, GeckoStyleContext* aStyleContext,
    nsPresContext* aPresContext, RuleNodeCacheConditions& aConditions) {
  nsCSSUnit unit = aValue.GetUnit();
  if (unit == eCSSUnit_Initial ||
      (UnsetTo == eUnsetInitial && unit == eCSSUnit_Unset)) {
  ...
  } else if (aValue.IsLengthUnit() || aValue.IsCalcUnit()) {
    nscoord len = CalcLength(aValue, aStyleContext, aPresContext, aConditions);
    if (len < 0) {
      NS_ASSERTION(aValue.IsCalcUnit(),
...
static void SetGradientCoord(const nsCSSValue& aValue,
                             nsPresContext* aPresContext,
...
static void SetGradient(const nsCSSValue& aValue, nsPresContext* aPresContext,
  ...
  if (gradient->mIsExplicitSize) {
    SetCoord(gradient->GetRadiusX(), aResult.mRadiusX, nsStyleCoord(),
             SETCOORD_LP | SETCOORD_STORE_CALC, aContext, aPresContext,
             aConditions);
  ...
  // angle
  const nsStyleCoord dummyParentCoord;
  if (!SetCoord(gradient->mAngle, aResult.mAngle, dummyParentCoord,
                SETCOORD_ANGLE, aContext, aPresContext, aConditions)) {
  ...
  for (uint32_t i = 0; i < gradient->mStops.Length(); i++) {
    ...
    if (!SetCoord(valueStop.mLocation, stop.mLocation, nsStyleCoord(),
                  SETCOORD_LPO | SETCOORD_STORE_CALC, aContext, aPresContext,
                  aConditions)) {
    ...
    if (!valueStop.mIsInterpolationHint) {
      SetColor(valueStop.mColor, NS_RGB(0, 0, 0), aPresContext, aContext,
               stop.mColor, aConditions);
    } else {
...
static void SetStyleImage(GeckoStyleContext* aStyleContext,
                          const nsCSSValue& aValue, nsStyleImage& aResult,
                          RuleNodeCacheConditions& aConditions) {
  if (aValue.GetUnit() == eCSSUnit_Null) {
    return;
  }
  ...
  switch (aValue.GetUnit()) {
    ...
    case eCSSUnit_URL: {
      ...
      MOZ_ASSERT(
          aStyleContext->IsStyleIfVisited() || isEqualExceptRef || isLocalRef,
...
template <typename FieldT>
struct SetValueHelper : Conditional<IsEnum<FieldT>::value, SetEnumValueHelper,
...
template <typename FieldT, typename InitialT, typename AutoT, typename NoneT,
          typename NormalT, typename SysFontT>
static void SetValue(const nsCSSValue& aValue, FieldT& aField,
                     RuleNodeCacheConditions& aConditions, uint32_t aMask,
                     FieldT aParentValue, InitialT aInitialValue,
                     AutoT aAutoValue, NoneT aNoneValue, NormalT aNormalValue,
...
static void SetValue(const nsCSSValue& aValue, FieldT& aField,
  ...
  SetValue(aValue, aField, aConditions, aMask, aParentValue, aInitialValue,
...
static void SetFactor(const nsCSSValue& aValue, float& aField,
                      RuleNodeCacheConditions& aConditions, float aParentValue,
                      float aInitialValue, uint32_t aFlags = 0) {
  switch (aValue.GetUnit()) {
    case eCSSUnit_Null:
      return;
    ...
    case eCSSUnit_Number:
      ...
      if (aFlags & SETFCT_POSITIVE) {
        NS_ASSERTION(aField >= 0.0f,
                     "negative value for positive-only property");
        if (aField < 0.0f) aField = 0.0f;
      }
      if (aFlags & SETFCT_OPACITY) {
        if (aField < 0.0f) aField = 0.0f;
        if (aField > 1.0f) aField = 1.0f;
      }
      return;
...
static void SetTransformValue(const nsCSSValue& aValue,
...
void* nsRuleNode::operator new(size_t sz, nsPresContext* aPresContext) {
  // Check the recycle list first.
  return aPresContext->PresShell()->AllocateByObjectID(
      eArenaObjectID_nsRuleNode, sz);
}

// Overridden to prevent the global delete from being called, since the memory
// came out of an nsIArena instead of the global delete operator's heap.
void nsRuleNode::Destroy() {
  // Destroy ourselves.
  this->~nsRuleNode();
...
already_AddRefed<nsRuleNode> nsRuleNode::CreateRootNode(
    nsPresContext* aPresContext) {
  return do_AddRef(new (aPresContext) nsRuleNode(aPresContext, nullptr, nullptr,
                                                 SheetType::Unknown, false));
}

nsRuleNode::nsRuleNode(nsPresContext* aContext, nsRuleNode* aParent,
                       nsIStyleRule* aRule, SheetType aLevel, bool aIsImportant)
    : mPresContext(aContext),
      ...
      mRefCnt(0) {
  MOZ_ASSERT(aContext);
  MOZ_ASSERT(IsRoot() == !aRule, "non-root rule nodes must have a rule");
...
nsRuleNode* nsRuleNode::Transition(nsIStyleRule* aRule, SheetType aLevel,
...
nsRuleNode* nsRuleNode::RuleTree() {
  nsRuleNode* n = this;
  while (n->mParent) {
    n = n->mParent;
  }
...
void nsRuleNode::ConvertChildrenToHash(int32_t aNumKids) {
  NS_ASSERTION(!ChildrenAreHashed() && HaveChildren(),
               "must have a non-empty list of children");
  PLDHashTable* hash =
      new PLDHashTable(&ChildrenHashOps, sizeof(ChildrenHashEntry), aNumKids);
  for (nsRuleNode* curr = ChildrenList(); curr; curr = curr->mNextSibling) {
    Key key = curr->GetKey();
    // This will never fail because of the initial size we gave the table.
    auto entry = static_cast<ChildrenHashEntry*>(hash->Add(&key));
...
void nsRuleNode::RemoveChild(nsRuleNode* aNode) {
  MOZ_ASSERT(HaveChildren());
  if (ChildrenAreHashed()) {
...
inline void nsRuleNode::PropagateNoneBit(uint32_t aBit,
                                         nsRuleNode* aHighestNode) {
  nsRuleNode* curr = this;
  for (;;) {
    NS_ASSERTION(!(curr->mNoneBits & aBit), "propagating too far");
    curr->mNoneBits |= aBit;
    if (curr == aHighestNode) break;
    curr = curr->mParent;
  }
}

inline void nsRuleNode::PropagateDependentBit(nsStyleStructID aSID,
                                              nsRuleNode* aHighestNode,
                                              void* aStruct) {
...
/* static */ void nsRuleNode::PropagateGrandancestorBit(
    GeckoStyleContext* aContext, GeckoStyleContext* aContextInheritedFrom) {
  MOZ_ASSERT(aContext);
  MOZ_ASSERT(aContextInheritedFrom && aContextInheritedFrom != aContext,
             "aContextInheritedFrom must be an ancestor of aContext");

  for (GeckoStyleContext* context = aContext->GetParent();
       context != aContextInheritedFrom; context = context->GetParent()) {
    if (!context) {
      MOZ_ASSERT(false,
                 "aContextInheritedFrom must be an ancestor of "
                 "aContext's parent");
      break;
    }
 ...
 */
typedef nsRuleNode::RuleDetail (*CheckCallbackFn)(
    const nsRuleData* aRuleData, nsRuleNode::RuleDetail aResult);

/**
 * @param aValue the value being examined
 * @param aSpecifiedCount to be incremented by one if the value is specified
 * @param aInheritedCount to be incremented by one if the value is set to
 * inherit
 * @param aUnsetCount to be incremented by one if the value is set to unset
 */
inline void ExamineCSSValue(const nsCSSValue& aValue, uint32_t& aSpecifiedCount,
                            uint32_t& aInheritedCount, uint32_t& aUnsetCount) {
  if (aValue.GetUnit() != eCSSUnit_Null) {
    ++aSpecifiedCount;
...
static nsRuleNode::RuleDetail CheckFontCallback(
    const nsRuleData* aRuleData, nsRuleNode::RuleDetail aResult) {
...
static nsRuleNode::RuleDetail CheckColorCallback(
    const nsRuleData* aRuleData, nsRuleNode::RuleDetail aResult) {
...
static nsRuleNode::RuleDetail CheckTextCallback(
    const nsRuleData* aRuleData, nsRuleNode::RuleDetail aResult) {
...
static nsRuleNode::RuleDetail CheckVariablesCallback(
    const nsRuleData* aRuleData, nsRuleNode::RuleDetail aResult) {
...
#define STYLE_STRUCT(name, checkdata_cb) g##name##Flags,
...
#define STYLE_STRUCT(name, checkdata_cb) checkdata_cb,
#include "nsStyleStructList.h"
#undef STYLE_STRUCT
...
#ifdef DEBUG
static bool AreAllMathMLPropertiesUndefined(const nsRuleData* aRuleData) {
  return aRuleData->ValueForScriptLevel()->GetUnit() == eCSSUnit_Null &&
         aRuleData->ValueForScriptSizeMultiplier()->GetUnit() ==
             eCSSUnit_Null &&
...
inline nsRuleNode::RuleDetail nsRuleNode::CheckSpecifiedProperties(
    const nsStyleStructID aSID, const nsRuleData* aRuleData) {
  ...
  for (nsCSSValue *
           values = aRuleData->mValueStorage,
...
// return 0.
inline uint32_t GetPseudoRestriction(GeckoStyleContext* aContext) {
...
static void UnsetPropertiesWithoutFlags(const nsStyleStructID aSID,
  ...
  for (size_t i = 0, i_end = nsCSSProps::PropertyCountInStruct(aSID);
       i != i_end; ++i) {
    if ((flagData[i] & aFlags) != aFlags) values[i].Reset();
  }
}
...
/* static */ bool nsRuleNode::ResolveVariableReferences(
    const nsStyleStructID aSID, nsRuleData* aRuleData,
  ...
  for (nsCSSValue *value = aRuleData->mValueStorage,
    ...
    parser.ParsePropertyWithVariableReferences(
        ...
        tokenStream->mTokenStream, variables, aRuleData, tokenStream->mSheetURI,
        tokenStream->mBaseURI, tokenStream->mSheetPrincipal, nullptr,
...
const void* nsRuleNode::WalkRuleTree(const nsStyleStructID aSID,
  ...
  nsRuleData ruleData(nsCachedStyleData::GetBitForSID(aSID), dataArray.get(),
                      aContext);
  ...
  while (ruleNode) {
    ...
    if (ruleNode->mNoneBits & bit) break;
    ...
    if (oldDetail == eRuleNone && detail != eRuleNone) highestNode = ruleNode;

    if (detail == eRuleFullReset || detail == eRuleFullMixed ||
  ...
  NS_ASSERTION(
      !startStruct || (detail != eRuleFullReset && detail != eRuleFullMixed &&
                       detail != eRuleFullInherited),
      "can't have start struct and be fully specified");

  bool isReset = nsCachedStyleData::IsReset(aSID);
  if (!highestNode) highestNode = rootNode;
  ...
  if (detail == eRuleNone && isReset) {
    ...
    if (ruleData.mConditions.CacheableWithDependencies()) {
      ...
      MOZ_ASSERT(
          ruleData.mConditions.CacheableWithoutDependencies() ||
              ((HasAnimationData() || GetLevel() == SheetType::Transition) &&
               aContext->GetParent() &&
  ...
  if (detail == eRuleNone && startStruct) {
    // We specified absolutely no rule information, but a parent rule in the
    // tree specified all the rule information.  We set a bit along the branch
    // from our node in the tree to the node that specified the data that tells
    // nodes on that branch that they never need to examine their rules for this
    // particular struct type ever again.
  ...
  if ((!startStruct && !isReset &&
    ...
    if (highestNode != this && !isReset) PropagateNoneBit(bit, highestNode);

    // All information must necessarily be inherited from our parent style
    // context. In the absence of any computed data in the rule tree and with no
    // rules specified that didn't have values of 'inherit', we should check our
    // parent.
    ...
    if (parentContext) {
      ...
      // Set the inherit bits on our context.  These bits tell the style context
      // that it never has to go back to the rule tree for data.  Instead the
      // style context tree should be walked to find the data.
  ...
  typedef const void* (nsRuleNode::*ComputeFunc)(
      void*, const nsRuleData*, GeckoStyleContext*, nsRuleNode*, RuleDetail,
...
const void* nsRuleNode::SetDefaultOnRoot(const nsStyleStructID aSID,
  ...
  switch (aSID) {
    ...
    case eStyleStruct_Visibility: {
      nsStyleVisibility* vis =
          new (mPresContext) nsStyleVisibility(mPresContext);
    ...
    case eStyleStruct_TextReset: {
      nsStyleTextReset* text =
          new (mPresContext) nsStyleTextReset(mPresContext);
    ...
    case eStyleStruct_Background: {
      nsStyleBackground* bg =
          new (mPresContext) nsStyleBackground(mPresContext);
    ...
    case eStyleStruct_TableBorder: {
      nsStyleTableBorder* table =
          new (mPresContext) nsStyleTableBorder(mPresContext);
    ...
    case eStyleStruct_UserInterface: {
      nsStyleUserInterface* ui =
          new (mPresContext) nsStyleUserInterface(mPresContext);
    ...
    case eStyleStruct_SVGReset: {
      nsStyleSVGReset* svgReset =
          new (mPresContext) nsStyleSVGReset(mPresContext);
      aContext->SetStyle(eStyleStruct_SVGReset, svgReset);
      return svgReset;
    }
    case eStyleStruct_Variables: {
      nsStyleVariables* vars =
          new (mPresContext) nsStyleVariables(mPresContext);
  ...
  if (eStyleStruct_##type_ == eStyleStruct_Variables)                        \
    ...
    if (aRuleDetail != eRuleFullMixed && aRuleDetail != eRuleFullReset) {    \
      ...
      data_ = new (mPresContext) nsStyle##type_(mPresContext);               \
                                                                             ...
                                                                             \
  if (!parentdata_) parentdata_ = data_;
...
#define COMPUTE_START_RESET(type_, data_, parentdata_)                       \
  ...
  if (aRuleDetail != eRuleFullReset && aRuleDetail != eRulePartialReset &&   \
...
#define COMPUTE_END_RESET(type_, data_)                                       \
  ...
  if (conditions.CacheableWithoutDependencies()) {                            \
    ...
    NS_ASSERTION(!aHighestNode->mStyleData.mResetData->GetStyleData(          \
                     eStyleStruct_##type_),                                   \
                 "Going to leak style data");                                 \
    aHighestNode->mStyleData.mResetData->SetStyleData(eStyleStruct_##type_,   \
                                                      data_);                 \
    /* Propagate the bit down. */                                             \
    PropagateDependentBit(eStyleStruct_##type_, aHighestNode, data_);         \
    if (HasAnimationData()) {                                                 \
    ...
    mStyleData.mResetData->SetStyleData(eStyleStruct_##type_, mPresContext,   \
                                        data_, conditions);                   \
    /* Tell the style context that it doesn't own the data */                 \
    aContext->AddStyleBit(NS_STYLE_INHERIT_BIT(type_));                       \
    ...
    if (aContext->GetParent()) {                                              \
      ...
      aContext->GetParent()->AddStyleBit(                                     \
          NS_STYLE_HAS_CHILD_THAT_USES_RESET_STYLE);                          \
    }                                                                         \
  }                                                                           \
                                                                              \
...
static nscoord ComputeScriptLevelSize(const nsStyleFont* aFont,
                                      const nsStyleFont* aParentFont,
                                      nsPresContext* aPresContext,
                                      nscoord* aUnconstrainedSize) {
  int32_t scriptLevelChange = aFont->mScriptLevel - aParentFont->mScriptLevel;
  ...
  *aUnconstrainedSize = NSToCoordRoundWithClamp(
      aParentFont->mScriptUnconstrainedSize * scriptLevelScale);
  ...
  if (scriptLevelScale <= 1.0) {
    ...
    NS_ASSERTION(*aUnconstrainedSize <= scriptLevelSize,
                 "How can this ever happen?");
    // Apply constraint #2
    return std::min(scriptLevelSize,
                    std::max(*aUnconstrainedSize, minScriptSize));
  }
}

/* static */ nscoord nsRuleNode::CalcFontPointSize(
...
#define sFontSizeTableMax 16
  ...
  // This table seems to be the one used by MacIE5. We hope its adoption in
  // Mozilla and eventually in WinIE5.5 will help to establish a standard
  // rendering across platforms and browsers. For now, it is used only in Strict
  // mode. More can be read in the document written by Todd Farhner at:
  // http://style.verso.com/font_size_intervals/altintervals.html
  //
  static int32_t
      sStrictFontSizeTable[sFontSizeTableMax - sFontSizeTableMin + 1][8] = {
          ...
          {9, 10, 13, 15, 18, 23, 30, 45}, {9, 10, 13, 16, 18, 24, 32, 48}};
  // HTML       1      2      3      4      5      6      7
  ...
  //
  static int32_t
      sQuirksFontSizeTable[sFontSizeTableMax - sFontSizeTableMin + 1][8] = {
...
//
  ...
  if ((fontSize >= sFontSizeTableMin) && (fontSize <= sFontSizeTableMax)) {
    ...
    if (aPresContext->CompatibilityMode() == eCompatibility_NavQuirks) {
      dFontSize = nsPresContext::CSSPixelsToAppUnits(
          sQuirksFontSizeTable[row][column[aHTMLSize]]);
    } else {
      dFontSize = nsPresContext::CSSPixelsToAppUnits(
          sStrictFontSizeTable[row][column[aHTMLSize]]);
    }
  } else {
...
struct SetFontSizeCalcOps : public css::BasicCoordCalcOps,
  ...
  SetFontSizeCalcOps(nscoord aParentSize, const nsStyleFont* aParentFont,
                     nsPresContext* aPresContext,
                     GeckoStyleContext* aStyleContext, bool aAtRoot,
                     RuleNodeCacheConditions& aConditions)
  ...
  bool ComputeLeafValue(result_type& aResult, const nsCSSValue& aValue) {
    ...
    if (aValue.IsLengthUnit()) {
      ...
      size = CalcLengthWith(aValue, mParentSize, mParentFont, mStyleContext,
                            mPresContext, mAtRoot, true, mConditions);
...
/* static */ void nsRuleNode::SetFontSize(
    nsPresContext* aPresContext, GeckoStyleContext* aContext,
    const nsRuleData* aRuleData, const nsStyleFont* aFont,
    const nsStyleFont* aParentFont, nscoord* aSize, const nsFont& aSystemFont,
    nscoord aParentSize, nscoord aScriptLevelAdjustedParentSize,
    bool aUsedStartStruct, bool aAtRoot, RuleNodeCacheConditions& aConditions) {
  ...
  int32_t baseSize =
      (int32_t)aPresContext->GetDefaultFont(aFont->mGenericID, aFont->mLanguage)
          ->size;
  ...
  if (eCSSUnit_Enumerated == sizeValue->GetUnit()) {
    ...
    if ((NS_STYLE_FONT_SIZE_XXSMALL <= value) &&
      ...
      *aSize = CalcFontPointSize(value, baseSize, aPresContext, eFontSize_CSS);
      ...
      *aSize = CalcFontPointSize(value, baseSize, aPresContext);
      ...
      float factor =
          (NS_STYLE_FONT_SIZE_LARGER == value) ? 1.2f : (1.0f / 1.2f);
    ...
    SetFontSizeCalcOps ops(aParentSize, aParentFont, aPresContext, aContext,
                           aAtRoot, aConditions);
  ...
  bool currentlyZoomed =
      sizeIsZoomedAccordingToParent && aParentFont->mAllowZoom;
...
static int8_t ClampTo8Bit(int32_t aValue) {
  if (aValue < -128) return -128;
  if (aValue > 127) return 127;
...
/* static */ void nsRuleNode::SetFont(nsPresContext* aPresContext,
  ...
  const nsFont* defaultVariableFont = aPresContext->GetDefaultFont(
      kPresContext_DefaultVariableFont_ID, aFont->mLanguage);
  ...
  SetValue(*aRuleData->ValueForMathVariant(), aFont->mMathVariant, aConditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, aParentFont->mMathVariant,
           NS_MATHML_MATHVARIANT_NONE);

  // -moz-math-display: enum, inherit, initial
  SetValue(*aRuleData->ValueForMathDisplay(), aFont->mMathDisplay, aConditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, aParentFont->mMathDisplay,
           NS_MATHML_DISPLAYSTYLE_INLINE);

  // -moz-osx-font-smoothing: enum, inherit, initial
  SetValue(*aRuleData->ValueForOsxFontSmoothing(), aFont->mFont.smoothing,
           aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           aParentFont->mFont.smoothing, defaultVariableFont->smoothing);
  ...
  if (aFont->mMathVariant != NS_MATHML_MATHVARIANT_NONE) {
    ...
    SetValue(*aRuleData->ValueForFontStyle(), aFont->mFont.style, aConditions,
             SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, aParentFont->mFont.style,
             defaultVariableFont->style, Unused, Unused, Unused,
             systemFont.style);
  }
  ...
  if (aFont->mMathVariant != NS_MATHML_MATHVARIANT_NONE) {
    ...
    SetValue(*weightValue, aFont->mFont.weight, aConditions,
             SETVAL_INTEGER | SETVAL_UNSET_INHERIT, aParentFont->mFont.weight,
             defaultVariableFont->weight, Unused, Unused, Unused,
             systemFont.weight);

  // font-stretch: enum, inherit, initial, -moz-system-font
  SetValue(*aRuleData->ValueForFontStretch(), aFont->mFont.stretch, aConditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, aParentFont->mFont.stretch,
           defaultVariableFont->stretch, Unused, Unused, Unused,
           systemFont.stretch);
  ...
  if (scriptMinSizeValue->IsLengthUnit()) {
    ...
    //
    aFont->mScriptMinSize = CalcLengthWith(
        *scriptMinSizeValue, aParentFont->mSize, aParentFont, aContext,
        aPresContext, atRoot, true /* aUseUserFontSet */, aConditions);
  }

  // -moz-script-size-multiplier: factor, inherit, initial
  SetFactor(*aRuleData->ValueForScriptSizeMultiplier(),
            aFont->mScriptSizeMultiplier, aConditions,
            aParentFont->mScriptSizeMultiplier,
  ...
  if (eCSSUnit_Integer == scriptLevelValue->GetUnit()) {
    ...
    aFont->mScriptLevel = ClampTo8Bit(aParentFont->mScriptLevel +
                                      scriptLevelValue->GetIntValue());
  } else if (eCSSUnit_Number == scriptLevelValue->GetUnit()) {
    // "absolute"
    aFont->mScriptLevel =
        ClampTo8Bit(int32_t(scriptLevelValue->GetFloatValue()));
  } else if (eCSSUnit_Auto == scriptLevelValue->GetUnit()) {
    // auto
    aConditions.SetUncacheable();
    aFont->mScriptLevel = ClampTo8Bit(
        aParentFont->mScriptLevel +
        (aParentFont->mMathDisplay == NS_MATHML_DISPLAYSTYLE_INLINE ? 1 : 0));
  ...
  SetValue(*aRuleData->ValueForFontKerning(), aFont->mFont.kerning, aConditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, aParentFont->mFont.kerning,
           defaultVariableFont->kerning, Unused, Unused, Unused,
           systemFont.kerning);

  // font-synthesis: none, enum (bit field), inherit, initial
  SetValue(*aRuleData->ValueForFontSynthesis(), aFont->mFont.synthesis,
           aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           aParentFont->mFont.synthesis, defaultVariableFont->synthesis, Unused,
           /* none */ 0, Unused, Unused);

  // font-optical-sizing: none, enum, inherit, initial, -moz-system-font
  SetValue(*aRuleData->ValueForFontOpticalSizing(), aFont->mFont.opticalSizing,
           aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           aParentFont->mFont.opticalSizing, defaultVariableFont->opticalSizing,
    ...
    case eCSSUnit_Pair:
      ...
      if (variantAlternates & NS_FONT_VARIANT_ALTERNATES_FUNCTIONAL_MASK) {
        ...
        aFont->mFont.featureValueLookup =
            aPresContext->GetFontFeatureValuesLookup();
  ...
  SetValue(*aRuleData->ValueForFontVariantCaps(), aFont->mFont.variantCaps,
           aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           aParentFont->mFont.variantCaps, defaultVariableFont->variantCaps,
  ...
  SetValue(*aRuleData->ValueForFontVariantEastAsian(),
           ...
           defaultVariableFont->variantEastAsian, Unused, Unused,
           /* normal */ 0, systemFont.variantEastAsian);
  ...
  SetValue(*aRuleData->ValueForFontVariantLigatures(),
           ...
           defaultVariableFont->variantLigatures, Unused,
           NS_FONT_VARIANT_LIGATURES_NONE, /* normal */ 0,
  ...
  SetValue(
      *aRuleData->ValueForFontVariantNumeric(), aFont->mFont.variantNumeric,
      aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
      aParentFont->mFont.variantNumeric, defaultVariableFont->variantNumeric,
      Unused, Unused, /* normal */ 0, systemFont.variantNumeric);

  // font-variant-position: normal, enum, inherit, initial,
  //                        -moz-system-font
  SetValue(
      *aRuleData->ValueForFontVariantPosition(), aFont->mFont.variantPosition,
      aConditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
      aParentFont->mFont.variantPosition, defaultVariableFont->variantPosition,
  ...
  NS_ASSERTION(
      !aUsedStartStruct || aFont->mScriptUnconstrainedSize == aFont->mSize,
      "If we have a start struct, we should have reset everything coming in "
      "here");
  ...
  SetFontSize(aPresContext, aContext, aRuleData, aFont, aParentFont,
              &aFont->mSize, systemFont, aParentFont->mSize,
              scriptLevelAdjustedParentSize, aUsedStartStruct, atRoot,
              aConditions);
  if (!aPresContext->Document()->GetMathMLEnabled()) {
    ...
    SetFontSize(aPresContext, aContext, aRuleData, aFont, aParentFont,
                ...
                scriptLevelAdjustedUnconstrainedParentSize, aUsedStartStruct,
                atRoot, unconstrainedConditions);
  }
  ...
  if (eCSSUnit_System_Font == sizeAdjustValue->GetUnit()) {
    ...
    SetFactor(*sizeAdjustValue, aFont->mFont.sizeAdjust, aConditions,
              aParentFont->mFont.sizeAdjust, -1.0f,
...
/* static */ void nsRuleNode::SetGenericFont(nsPresContext* aPresContext,
  ...
  *aFont = parentFont;
  ...
  for (int32_t i = contextPath.Length() - 1; i >= 0; --i) {
    ...
    if (i != 0) ruleData.ValueForFontFamily()->Reset();

    ResolveVariableReferences(eStyleStruct_Font, &ruleData, aContext);

    RuleNodeCacheConditions dummy;
    nsRuleNode::SetFont(aPresContext, context, aGenericFontID, &ruleData,
                        &parentFont, aFont, false, dummy);
...
const void* nsRuleNode::ComputeFontData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  if (generic == kGenericFont_NONE) {
    ...
    nsRuleNode::SetFont(mPresContext, aContext, generic, aRuleData, parentFont,
                        font, aStartStruct != nullptr, conditions);
  } else {
    // re-calculate the font as a generic font
    conditions.SetUncacheable();
    nsRuleNode::SetGenericFont(mPresContext, aContext, generic, font);
  }
...
static already_AddRefed<nsCSSShadowArray> GetShadowData(
    const nsCSSValueList* aList, GeckoStyleContext* aContext, bool aIsBoxShadow,
    nsPresContext* aPresContext, RuleNodeCacheConditions& aConditions) {
  ...
  if (!shadowList) return nullptr;

  nsStyleCoord tempCoord;
  DebugOnly<bool> unitOK;
  for (nsCSSShadowItem *item = shadowList->ShadowAt(0); aList;
    ...
    unitOK = SetCoord(arr->Item(0), tempCoord, nsStyleCoord(),
                      SETCOORD_LENGTH | SETCOORD_CALC_LENGTH_ONLY, aContext,
                      aPresContext, aConditions);
    ...
    if (aIsBoxShadow && arr->Item(5).GetUnit() == eCSSUnit_Enumerated) {
      NS_ASSERTION(
          arr->Item(5).GetIntValue() == uint8_t(StyleBoxShadowType::Inset),
...
struct LengthNumberCalcOps : public css::FloatCoeffsAlreadyNormalizedOps {
  ...
  bool ComputeLeafValue(result_type& aResult, const nsCSSValue& aValue) {
    if (aValue.IsLengthUnit()) {
      ...
      aResult.mValue =
          CalcLength(aValue, mStyleContext, mPresContext, mConditions);
...
const void* nsRuleNode::ComputeTextData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  if (tabSizeValue->GetUnit() == eCSSUnit_Initial) {
    ...
    if (obj.mIsNumber) {
      ...
      text->mTabSize.SetCoordValue(NSToCoordRoundWithClamp(value));
    }
  } else {
    SetCoord(*tabSizeValue, text->mTabSize, parentText->mTabSize,
             SETCOORD_LH | SETCOORD_FACTOR | SETCOORD_UNSET_INHERIT, aContext,
             mPresContext, conditions);
  }

  // letter-spacing: normal, length, inherit
  SetCoord(*aRuleData->ValueForLetterSpacing(), text->mLetterSpacing,
           parentText->mLetterSpacing,
  ...
  if (textShadowValue->GetUnit() != eCSSUnit_Null) {
    ...
    if (textShadowValue->GetUnit() == eCSSUnit_Inherit ||
      ...
      // List of arrays
      text->mTextShadow =
          GetShadowData(textShadowValue->GetListValue(), aContext, false,
                        mPresContext, conditions);
    }
  }
  ...
  if (eCSSUnit_Percent == lineHeightValue->GetUnit()) {
    ...
    if (obj.mIsNumber) {
      ...
      text->mLineHeight.SetCoordValue(NSToCoordRoundWithClamp(obj.mValue));
    }
  } else {
    ...
    if (lineHeightValue->IsLengthUnit() &&
      ...
      if (minimumFontSize > 0 && !mPresContext->IsChrome()) {
        if (font->mSize != 0) {
          lh =
              nscoord(float(lh) * float(font->mFont.size) / float(font->mSize));
        } else {
          lh = minimumFontSize;
        }
      }
  ...
  if (eCSSUnit_String == textAlignValue->GetUnit()) {
    ...
    SetValue(*textAlignValue, text->mTextAlign, conditions,
             SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mTextAlign,
  ...
  if (eCSSUnit_Pair == textAlignLastValue->GetUnit()) {
    ...
    const nsCSSValuePair& textAlignLastValuePair =
        textAlignLastValue->GetPairValue();
  ...
  SetValue(*textAlignLastValue, text->mTextAlignLast, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mTextAlignLast,
           NS_STYLE_TEXT_ALIGN_AUTO);

  // text-indent: length, percent, calc, inherit, initial
  SetCoord(*aRuleData->ValueForTextIndent(), text->mTextIndent,
           parentText->mTextIndent,
  ...
  SetValue(*aRuleData->ValueForTextJustify(), text->mTextJustify, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mTextJustify,
           StyleTextJustify::Auto);

  // text-transform: enum, inherit, initial
  SetValue(*aRuleData->ValueForTextTransform(), text->mTextTransform,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
  ...
  SetValue(*aRuleData->ValueForWhiteSpace(), text->mWhiteSpace, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mWhiteSpace,
           StyleWhiteSpace::Normal);

  // word-break: enum, inherit, initial
  SetValue(*aRuleData->ValueForWordBreak(), text->mWordBreak, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mWordBreak,
  ...
  if (wordSpacingValue->GetUnit() == eCSSUnit_Normal) {
    ...
    SetCoord(*aRuleData->ValueForWordSpacing(), text->mWordSpacing,
             parentText->mWordSpacing,
             SETCOORD_LPH | SETCOORD_INITIAL_ZERO | SETCOORD_STORE_CALC |
                 SETCOORD_UNSET_INHERIT,
             aContext, mPresContext, conditions);
  }
  ...
  SetValue(*aRuleData->ValueForOverflowWrap(), text->mOverflowWrap, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mOverflowWrap,
           NS_STYLE_OVERFLOWWRAP_NORMAL);

  // hyphens: enum, inherit, initial
  SetValue(*aRuleData->ValueForHyphens(), text->mHyphens, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mHyphens,
           StyleHyphens::Manual);

  // ruby-align: enum, inherit, initial
  SetValue(*aRuleData->ValueForRubyAlign(), text->mRubyAlign, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mRubyAlign,
           NS_STYLE_RUBY_ALIGN_SPACE_AROUND);

  // ruby-position: enum, inherit, initial
  SetValue(*aRuleData->ValueForRubyPosition(), text->mRubyPosition, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentText->mRubyPosition,
  ...
  SetValue(*aRuleData->ValueForTextSizeAdjust(), text->mTextSizeAdjust,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentText->mTextSizeAdjust, NS_STYLE_TEXT_SIZE_ADJUST_AUTO);

  // text-combine-upright: enum, inherit, initial
  SetValue(*aRuleData->ValueForTextCombineUpright(), text->mTextCombineUpright,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentText->mTextCombineUpright, NS_STYLE_TEXT_COMBINE_UPRIGHT_NONE);
  ...
  SetValue(*aRuleData->ValueForTextEmphasisPosition(),
           text->mTextEmphasisPosition, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
  ...
  SetValue(*aRuleData->ValueForTextRendering(), text->mTextRendering,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentText->mTextRendering, NS_STYLE_TEXT_RENDERING_AUTO);
  ...
  Maybe<nscoord> coord = ComputeLineWidthValue<eUnsetInherit>(
      *aRuleData->ValueForWebkitTextStrokeWidth(),
      parentText->mWebkitTextStrokeWidth, 0, aContext, mPresContext,
      conditions);
  if (coord.isSome()) {
  ...
  SetValue(*aRuleData->ValueForControlCharacterVisibility(),
           text->mControlCharacterVisibility, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
...
const void* nsRuleNode::ComputeTextResetData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetComplexColor<eUnsetInitial>(
      *aRuleData->ValueForTextDecorationColor(),
      parentText->mTextDecorationColor, StyleComplexColor::CurrentColor(),
      mPresContext, text->mTextDecorationColor, conditions);
  ...
  const nsCSSValue* textOverflowValue = aRuleData->ValueForTextOverflow();
  if (eCSSUnit_Initial == textOverflowValue->GetUnit() ||
    ...
    SetValue(*textOverflowValue, text->mTextOverflow.mRight.mType, conditions,
    ...
    if (eCSSUnit_Enumerated == textOverflowLeftValue->GetUnit()) {
      SetValue(*textOverflowLeftValue, text->mTextOverflow.mLeft.mType,
               conditions, SETVAL_ENUMERATED,
               parentText->mTextOverflow.mLeft.mType,
               NS_STYLE_TEXT_OVERFLOW_CLIP);
      text->mTextOverflow.mLeft.mString.Truncate();
    ...
    if (eCSSUnit_Enumerated == textOverflowRightValue->GetUnit()) {
      SetValue(*textOverflowRightValue, text->mTextOverflow.mRight.mType,
               conditions, SETVAL_ENUMERATED,
               parentText->mTextOverflow.mRight.mType,
               NS_STYLE_TEXT_OVERFLOW_CLIP);
      text->mTextOverflow.mRight.mString.Truncate();
    } else if (eCSSUnit_String == textOverflowRightValue->GetUnit()) {
      textOverflowRightValue->GetStringValue(
          text->mTextOverflow.mRight.mString);
      text->mTextOverflow.mRight.mType = NS_STYLE_TEXT_OVERFLOW_STRING;
    }
  }
  ...
  SetValue(*aRuleData->ValueForUnicodeBidi(), text->mUnicodeBidi, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentText->mUnicodeBidi,
  ...
  if (initialLetterValue->GetUnit() == eCSSUnit_Null) {
    ...
    const nsCSSValue& secondValue =
        initialLetterValue->GetArrayValue()->Item(1);
...
const void* nsRuleNode::ComputeUserInterfaceData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  if (cursorUnit != eCSSUnit_Null) {
    ...
    if (cursorUnit == eCSSUnit_Inherit || cursorUnit == eCSSUnit_Unset) {
      conditions.SetUncacheable();
  ...
  SetValue(*aRuleData->ValueForUserInput(), ui->mUserInput, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentUI->mUserInput,
           StyleUserInput::Auto);

  // user-modify: enum, inherit, initial
  SetValue(*aRuleData->ValueForUserModify(), ui->mUserModify, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentUI->mUserModify,
           StyleUserModify::ReadOnly);

  // user-focus: enum, inherit, initial
  SetValue(*aRuleData->ValueForUserFocus(), ui->mUserFocus, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentUI->mUserFocus,
           StyleUserFocus::None);

  // pointer-events: enum, inherit, initial
  SetValue(*aRuleData->ValueForPointerEvents(), ui->mPointerEvents, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentUI->mPointerEvents,
           NS_STYLE_POINTER_EVENTS_AUTO);

  // caret-color: auto, color, inherit
  const nsCSSValue* caretColorValue = aRuleData->ValueForCaretColor();
  SetComplexColor<eUnsetInherit>(*caretColorValue, parentUI->mCaretColor,
                                 StyleComplexColor::Auto(), mPresContext,
                                 ui->mCaretColor, conditions);

  COMPUTE_END_INHERITED(UserInterface, ui)
}

const void* nsRuleNode::ComputeUIResetData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForUserSelect(), ui->mUserSelect, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentUI->mUserSelect,
           StyleUserSelect::Auto);

  // ime-mode: enum, inherit, initial
  SetValue(*aRuleData->ValueForImeMode(), ui->mIMEMode, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentUI->mIMEMode,
           NS_STYLE_IME_MODE_AUTO);

  // force-broken-image-icons: integer, inherit, initial
  SetValue(*aRuleData->ValueForForceBrokenImageIcon(),
           ui->mForceBrokenImageIcon, conditions,
           SETVAL_INTEGER | SETVAL_UNSET_INITIAL,
           parentUI->mForceBrokenImageIcon, 0);

  // -moz-window-dragging: enum, inherit, initial
  SetValue(*aRuleData->ValueForWindowDragging(), ui->mWindowDragging,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentUI->mWindowDragging, StyleWindowDragging::Default);

  // -moz-window-shadow: enum, inherit, initial
  SetValue(*aRuleData->ValueForWindowShadow(), ui->mWindowShadow, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentUI->mWindowShadow,
           NS_STYLE_WINDOW_SHADOW_DEFAULT);

  // -moz-window-opacity: factor, inherit, initial
  SetFactor(*aRuleData->ValueForWindowOpacity(), ui->mWindowOpacity, conditions,
  ...
  if (windowTransformOriginValue->GetUnit() != eCSSUnit_Null) {
    mozilla::DebugOnly<bool> result = SetPairCoords(
        *windowTransformOriginValue, ui->mWindowTransformOrigin[0],
        ui->mWindowTransformOrigin[1], parentUI->mWindowTransformOrigin[0],
...
static const TransitionPropInfo transitionPropInfo[4] = {
    {eCSSProperty_transition_delay, &nsStyleDisplay::mTransitionDelayCount},
...
static const TransitionPropInfo animationPropInfo[8] = {
    {eCSSProperty_animation_delay, &nsStyleDisplay::mAnimationDelayCount},
    {eCSSProperty_animation_duration, &nsStyleDisplay::mAnimationDurationCount},
    {eCSSProperty_animation_name, &nsStyleDisplay::mAnimationNameCount},
...
static uint32_t CountTransitionProps(const TransitionPropInfo* aInfo,
                                     TransitionPropData* aData, size_t aLength,
  ...
  for (size_t i = 0; i < aLength; ++i) {
    ...
    if (data.num > numTransitions) numTransitions = data.num;
  }

  return numTransitions;
}

/* static */ void nsRuleNode::ComputeTimingFunction(const nsCSSValue& aValue,
  ...
  switch (aValue.GetUnit()) {
    ...
    case eCSSUnit_Cubic_Bezier: {
      ...
      aResult = nsTimingFunction(
          array->Item(0).GetFloatValue(), array->Item(1).GetFloatValue(),
          array->Item(2).GetFloatValue(), array->Item(3).GetFloatValue());
    } break;
    case eCSSUnit_Steps: {
      nsCSSValue::Array* array = aValue.GetArrayValue();
...
static uint8_t GetWillChangeBitFieldFromPropFlags(
    const nsCSSPropertyID& aProp) {
...
const void* nsRuleNode::ComputeDisplayData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
...
#define FOR_ALL_TRANSITION_PROPS(var_) for (uint32_t var_ = 0; var_ < 4; ++var_)

  // CSS Transitions
  uint32_t numTransitions = CountTransitionProps(
      transitionPropInfo, transitionPropData, ArrayLength(transitionPropData),
      display, parentDisplay, aRuleData, conditions);
  ...
  for (uint32_t i = 0; i < numTransitions; ++i) {
    ...
    } else if (delay.unit == eCSSUnit_Initial || delay.unit == eCSSUnit_Unset) {
    ...
    if (i >= property.num) {
      ...
      if (val.GetUnit() == eCSSUnit_Ident) {
        nsDependentString propertyStr(
            property.list->mValue.GetStringBufferValue());
        nsCSSPropertyID prop = nsCSSProps::LookupProperty(
            propertyStr, CSSEnabledState::eForAllContent);
...
#define FOR_ALL_ANIMATION_PROPS(var_) for (uint32_t var_ = 0; var_ < 8; ++var_)

  // CSS Animations.

  uint32_t numAnimations = CountTransitionProps(
      animationPropInfo, animationPropData, ArrayLength(animationPropData),
      display, parentDisplay, aRuleData, conditions);
  ...
  for (uint32_t i = 0; i < numAnimations; ++i) {
    ...
    if (i >= animDirection.num) {
      MOZ_ASSERT(animDirection.num, "animDirection.num must be greater than 0");
      animation->SetDirection(
          display->mAnimations[i % animDirection.num].GetDirection());
      ...
      animation->SetDirection(static_cast<dom::PlaybackDirection>(
          animDirection.list->mValue.GetIntValue()));
    }

    if (i >= animFillMode.num) {
      MOZ_ASSERT(animFillMode.num, "animFillMode.num must be greater than 0");
      animation->SetFillMode(
          display->mAnimations[i % animFillMode.num].GetFillMode());
    ...
    if (i >= animPlayState.num) {
      MOZ_ASSERT(animPlayState.num, "animPlayState.num must be greater than 0");
      animation->SetPlayState(
          display->mAnimations[i % animPlayState.num].GetPlayState());
    ...
    if (i >= animIterationCount.num) {
      ...
      animation->SetIterationCount(
          display->mAnimations[i % animIterationCount.num].GetIterationCount());
      ...
      MOZ_ASSERT(!conditions.Cacheable(),
                 "should have made conditions.Cacheable() false above");
      animation->SetIterationCount(
          parentDisplay->mAnimations[i].GetIterationCount());
      ...
      switch (animIterationCount.list->mValue.GetUnit()) {
        ...
        case eCSSUnit_Number:
          ...
          break;
        default:
          MOZ_ASSERT(false, "unexpected animation-iteration-count unit");
      }
    }
  ...
  SetValue(*aRuleData->ValueForDisplay(), display->mDisplay, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mDisplay,
           StyleDisplay::Inline);

  // contain: none, enum, inherit, initial
  SetValue(*aRuleData->ValueForContain(), display->mContain, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mContain,
           NS_STYLE_CONTAIN_NONE, Unused, NS_STYLE_CONTAIN_NONE, Unused,
           Unused);
  ...
  SetValue(*aRuleData->ValueForOverscrollBehaviorX(),
           display->mOverscrollBehaviorX, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mOverscrollBehaviorX, StyleOverscrollBehavior::Auto);

  // overscroll-behavior-y: none, enum, inherit, initial
  SetValue(*aRuleData->ValueForOverscrollBehaviorY(),
           display->mOverscrollBehaviorY, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mOverscrollBehaviorY, StyleOverscrollBehavior::Auto);
  ...
  const nsCSSValue& snapDestination =
      *aRuleData->ValueForScrollSnapDestination();
  ...
  SetValue(*aRuleData->ValueForIsolation(), display->mIsolation, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mIsolation,
           NS_STYLE_ISOLATION_AUTO);

  // -moz-top-layer: enum, inherit, initial
  SetValue(*aRuleData->ValueForTopLayer(), display->mTopLayer, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mTopLayer,
           NS_STYLE_TOP_LAYER_NONE);

  // Backup original display value for calculation of a hypothetical
  // box (CSS2 10.6.4/10.6.5), in addition to getting our style data right
  // later. See ReflowInput::CalculateHypotheticalBox
  display->mOriginalDisplay = display->mDisplay;

  // appearance: enum, inherit, initial
  SetValue(*aRuleData->ValueForAppearance(), display->mAppearance, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mAppearance,
           NS_THEME_NONE);
  ...
  SetValue(*aRuleData->ValueForPosition(), display->mPosition, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mPosition,
  ...
  SetValue(*aRuleData->ValueForClear(), display->mBreakType, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mBreakType,
           StyleClear::None);
  ...
  SetValue(*aRuleData->ValueForPageBreakInside(), display->mBreakInside,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mBreakInside, NS_STYLE_PAGE_BREAK_AUTO);

  // touch-action: none, auto, enum, inherit, initial
  SetValue(*aRuleData->ValueForTouchAction(), display->mTouchAction, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
  ...
  SetValue(*aRuleData->ValueForFloat(), display->mFloat, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mFloat,
           StyleFloat::None);
  ...
  SetValue(*aRuleData->ValueForOverflowX(), display->mOverflowX, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mOverflowX,
           NS_STYLE_OVERFLOW_VISIBLE);

  // overflow-y: enum, inherit, initial
  SetValue(*aRuleData->ValueForOverflowY(), display->mOverflowY, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mOverflowY,
           NS_STYLE_OVERFLOW_VISIBLE);
  ...
  SetValue(*aRuleData->ValueForOverflowClipBoxBlock(),
           display->mOverflowClipBoxBlock, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
  ...
  SetValue(*aRuleData->ValueForOverflowClipBoxInline(),
           display->mOverflowClipBoxInline, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mOverflowClipBoxInline,
           NS_STYLE_OVERFLOW_CLIP_BOX_PADDING_BOX);

  SetValue(*aRuleData->ValueForResize(), display->mResize, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mResize,
  ...
  if (display->mDisplay != StyleDisplay::None) {
    ...
    if (pseudo && display->mDisplay == StyleDisplay::Contents) {
      ...
      // We might get display:contents to work for
      // CSS_PSEUDO_ELEMENT_CONTAINS_ELEMENTS pseudos (:first-letter etc) in the
      // future, but those have a lot of special handling in frame construction
      // so they are also unsupported for now.
    ...
    if (nsCSSPseudoElements::firstLetter == pseudo) {
      ...
      // Note that we reset mOriginalDisplay to enforce the invariant that it
      // equals mDisplay if we're not positioned or floating.
      display->mOriginalDisplay = display->mDisplay = StyleDisplay::Inline;
  ...
  switch (willChangeValue->GetUnit()) {
    ...
    case eCSSUnit_ListDep: {
      ...
      for (const nsCSSValueList* item = willChangeValue->GetListValue(); item;
           item = item->mNext) {
          ...
          if (nsCSSProps::IsShorthand(prop)) {
            for (const nsCSSPropertyID* shorthands =
              ...
              display->mWillChangeBitField |=
                  GetWillChangeBitFieldFromPropFlags(*shorthands);
            }
          } else {
            display->mWillChangeBitField |=
                GetWillChangeBitFieldFromPropFlags(prop);
          }
        }
      }
      break;
  ...
  const nsCSSValue* transformOriginValue = aRuleData->ValueForTransformOrigin();
  if (transformOriginValue->GetUnit() != eCSSUnit_Null) {
    const nsCSSValue& valX =
        transformOriginValue->GetUnit() == eCSSUnit_Triplet
            ? transformOriginValue->GetTripletValue().mXValue
            : *transformOriginValue;
    const nsCSSValue& valY =
        transformOriginValue->GetUnit() == eCSSUnit_Triplet
            ? transformOriginValue->GetTripletValue().mYValue
            : *transformOriginValue;
    const nsCSSValue& valZ =
        transformOriginValue->GetUnit() == eCSSUnit_Triplet
            ? transformOriginValue->GetTripletValue().mZValue
            : *transformOriginValue;

    mozilla::DebugOnly<bool> cX = SetCoord(
        valX, display->mTransformOrigin[0], parentDisplay->mTransformOrigin[0],
        SETCOORD_LPH | SETCOORD_INITIAL_HALF | SETCOORD_BOX_POSITION |
            SETCOORD_STORE_CALC | SETCOORD_UNSET_INITIAL,
        aContext, mPresContext, conditions);

    mozilla::DebugOnly<bool> cY = SetCoord(
        valY, display->mTransformOrigin[1], parentDisplay->mTransformOrigin[1],
        SETCOORD_LPH | SETCOORD_INITIAL_HALF | SETCOORD_BOX_POSITION |
            SETCOORD_STORE_CALC | SETCOORD_UNSET_INITIAL,
        aContext, mPresContext, conditions);
  ...
  if (perspectiveOriginValue->GetUnit() != eCSSUnit_Null) {
    mozilla::DebugOnly<bool> result = SetPairCoords(
        *perspectiveOriginValue, display->mPerspectiveOrigin[0],
        display->mPerspectiveOrigin[1], parentDisplay->mPerspectiveOrigin[0],
  ...
  SetCoord(*aRuleData->ValueForPerspective(), display->mChildPerspective,
           parentDisplay->mChildPerspective,
           SETCOORD_LAH | SETCOORD_INITIAL_NONE | SETCOORD_NONE |
               SETCOORD_UNSET_INITIAL,
           aContext, mPresContext, conditions);

  SetValue(
      *aRuleData->ValueForBackfaceVisibility(), display->mBackfaceVisibility,
      conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
      parentDisplay->mBackfaceVisibility, NS_STYLE_BACKFACE_VISIBILITY_VISIBLE);

  // transform-style: enum, inherit, initial
  SetValue(*aRuleData->ValueForTransformStyle(), display->mTransformStyle,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mTransformStyle, NS_STYLE_TRANSFORM_STYLE_FLAT);

  // transform-box: enum, inherit, initial
  SetValue(*aRuleData->ValueForTransformBox(), display->mTransformBox,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentDisplay->mTransformBox, StyleGeometryBox::BorderBox);

  // orient: enum, inherit, initial
  SetValue(*aRuleData->ValueForOrient(), display->mOrient, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentDisplay->mOrient,
           StyleOrient::Inline);
...
const void* nsRuleNode::ComputeVisibilityData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForDirection(), visibility->mDirection, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
  ...
  SetValue(*aRuleData->ValueForVisibility(), visibility->mVisible, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentVisibility->mVisible,
           NS_STYLE_VISIBILITY_VISIBLE);

  // image-rendering: enum, inherit
  SetValue(*aRuleData->ValueForImageRendering(), visibility->mImageRendering,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentVisibility->mImageRendering, NS_STYLE_IMAGE_RENDERING_AUTO);

  // writing-mode: enum, inherit, initial
  SetValue(*aRuleData->ValueForWritingMode(), visibility->mWritingMode,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentVisibility->mWritingMode, NS_STYLE_WRITING_MODE_HORIZONTAL_TB);

  // text-orientation: enum, inherit, initial
  SetValue(*aRuleData->ValueForTextOrientation(), visibility->mTextOrientation,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentVisibility->mTextOrientation, NS_STYLE_TEXT_ORIENTATION_MIXED);
  ...
  if (orientation->GetUnit() == eCSSUnit_Inherit ||
    ...
    MOZ_ASSERT(
        array->Item(1).GetUnit() == eCSSUnit_Enumerated &&
  ...
  SetValue(*aRuleData->ValueForColorAdjust(), visibility->mColorAdjust,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentVisibility->mColorAdjust, NS_STYLE_COLOR_ADJUST_ECONOMY);

  COMPUTE_END_INHERITED(Visibility, visibility)
}

const void* nsRuleNode::ComputeColorData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  // Special case for currentColor.  According to CSS3, setting color to
  // 'currentColor' should behave as if it is inherited
  const nsCSSValue* colorValue = aRuleData->ValueForColor();
  if ((colorValue->GetUnit() == eCSSUnit_EnumColor &&
       colorValue->GetIntValue() == NS_COLOR_CURRENTCOLOR) ||
...
struct BackgroundItemComputer<nsCSSValuePairList, nsStyleImageLayers::Repeat> {
  static void ComputeValue(GeckoStyleContext* aStyleContext,
                           ...
                           RuleNodeCacheConditions& aConditions) {
    NS_ASSERTION(
        aSpecifiedValue->mXValue.GetUnit() == eCSSUnit_Enumerated &&
            (aSpecifiedValue->mYValue.GetUnit() == eCSSUnit_Enumerated ||
             aSpecifiedValue->mYValue.GetUnit() == eCSSUnit_Null),
        "Invalid unit");

    bool hasContraction = true;
    StyleImageLayerRepeat value = static_cast<StyleImageLayerRepeat>(
        aSpecifiedValue->mXValue.GetIntValue());
    switch (value) {
      ...
      default:
        NS_ASSERTION(value == StyleImageLayerRepeat::NoRepeat ||
                         ...
                         value == StyleImageLayerRepeat::Round,
                     "Unexpected value");
    ...
    switch (aSpecifiedValue->mYValue.GetUnit()) {
      case eCSSUnit_Null:
        ...
        break;
      case eCSSUnit_Enumerated:
        value = static_cast<StyleImageLayerRepeat>(
            aSpecifiedValue->mYValue.GetIntValue());
        NS_ASSERTION(value == StyleImageLayerRepeat::NoRepeat ||
                         value == StyleImageLayerRepeat::Repeat ||
                         value == StyleImageLayerRepeat::Space ||
                         value == StyleImageLayerRepeat::Round,
...
struct BackgroundItemComputer<nsCSSValueList, T> {
  ...
  static void ComputeValue(GeckoStyleContext* aStyleContext,
                           ...
                           RuleNodeCacheConditions& aConditions) {
    aComputedValue = static_cast<T>(aSpecifiedValue->mValue.GetIntValue());
  }
};
 ...
 */
static void ComputePositionCoord(GeckoStyleContext* aStyleContext,
  ...
  if (eCSSUnit_Percent == aOffset.GetUnit()) {
    ...
    aResult->mLength = CalcLength(aOffset, aStyleContext,
                                  aStyleContext->PresContext(), aConditions);
  ...
  if (eCSSUnit_Enumerated == aEdge.GetUnit()) {
    ...
    aResult->mPercent =
        GetFloatFromBoxPosition(aEdge.GetIntValue()) + sign * aResult->mPercent;
...
static void ComputePositionCoordValue(GeckoStyleContext* aStyleContext,
  ...
  ComputePositionCoord(aStyleContext, edge, offset, &aComputedValue,
                       aConditions);
}
...
static const BackgroundSizeAxis gBGSizeAxes[] = {
    {&nsCSSValuePairList::mXValue, &nsStyleImageLayers::Size::mWidth,
     &nsStyleImageLayers::Size::mWidthType},
    {&nsCSSValuePairList::mYValue, &nsStyleImageLayers::Size::mHeight,
  ...
  static void ComputeValue(GeckoStyleContext* aStyleContext,
    ...
    for (const BackgroundSizeAxis *axis = gBGSizeAxes,
      ...
      if (eCSSUnit_Auto == specified.GetUnit()) {
        ...
        MOZ_ASSERT(
            specified.GetIntValue() == NS_STYLE_IMAGELAYER_SIZE_CONTAIN ||
    ...
    MOZ_ASSERT(size.mWidthType < nsStyleImageLayers::Size::eDimensionType_COUNT,
               "bad width type");
    MOZ_ASSERT(
        size.mHeightType < nsStyleImageLayers::Size::eDimensionType_COUNT,
        "bad height type");
...
static void SetImageLayerList(
    GeckoStyleContext* aStyleContext, const nsCSSValue& aValue,
    nsStyleAutoArray<nsStyleImageLayers::Layer>& aLayers,
    const nsStyleAutoArray<nsStyleImageLayers::Layer>& aParentLayers,
    ComputedValueItem nsStyleImageLayers::Layer::*aResultLocation,
    ComputedValueItem aInitialValue, uint32_t aParentItemCount,
    uint32_t& aItemCount, uint32_t& aMaxItemCount, bool& aRebuild,
    RuleNodeCacheConditions& aConditions) {
  switch (aValue.GetUnit()) {
    case eCSSUnit_Null:
      break;
    ...
    case eCSSUnit_ListDep: {
      ...
      do {
        ...
        BackgroundItemComputer<nsCSSValueList, ComputedValueItem>::ComputeValue(
            aStyleContext, item, aLayers[aItemCount - 1].*aResultLocation,
            aConditions);
        item = item->mNext;
      } while (item);
      break;
  ...
  if (aItemCount > aMaxItemCount) aMaxItemCount = aItemCount;
}
...
static void SetImageLayerPositionCoordList(
    GeckoStyleContext* aStyleContext, const nsCSSValue& aValue,
    nsStyleAutoArray<nsStyleImageLayers::Layer>& aLayers,
    const nsStyleAutoArray<nsStyleImageLayers::Layer>& aParentLayers,
    Position::Coord Position::*aResultLocation, Position::Coord aInitialValue,
    uint32_t aParentItemCount, uint32_t& aItemCount, uint32_t& aMaxItemCount,
    bool& aRebuild, RuleNodeCacheConditions& aConditions) {
  switch (aValue.GetUnit()) {
    case eCSSUnit_Null:
      break;
    ...
    case eCSSUnit_Inherit:
      ...
      aItemCount = aParentItemCount;
      for (uint32_t i = 0; i < aParentItemCount; ++i) {
        aLayers[i].mPosition.*aResultLocation =
            aParentLayers[i].mPosition.*aResultLocation;
      }
      break;
    ...
    case eCSSUnit_ListDep: {
      ...
      do {
        ...
        ComputePositionCoordValue(
            aStyleContext, item->mValue,
            aLayers[aItemCount - 1].mPosition.*aResultLocation, aConditions);
        item = item->mNext;
      } while (item);
      break;
    }
...
template <class ComputedValueItem>
static void SetImageLayerPairList(
    GeckoStyleContext* aStyleContext, const nsCSSValue& aValue,
    nsStyleAutoArray<nsStyleImageLayers::Layer>& aLayers,
    const nsStyleAutoArray<nsStyleImageLayers::Layer>& aParentLayers,
    ComputedValueItem nsStyleImageLayers::Layer::*aResultLocation,
      ...
      do {
        ...
        BackgroundItemComputer<nsCSSValuePairList, ComputedValueItem>::
            ComputeValue(aStyleContext, item,
                         aLayers[aItemCount - 1].*aResultLocation, aConditions);
        item = item->mNext;
      } while (item);
      break;
    }
...
const void* nsRuleNode::ComputeBackgroundData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetComplexColor<eUnsetInitial>(
      *aRuleData->ValueForBackgroundColor(), parentBG->mBackgroundColor,
      StyleComplexColor::FromColor(NS_RGBA(0, 0, 0, 0)), mPresContext,
  ...
  SetImageLayerList(aContext, *aRuleData->ValueForBackgroundImage(),
                    ...
                    &nsStyleImageLayers::Layer::mImage, initialImage,
                    parentBG->mImage.mImageCount, bg->mImage.mImageCount,
  ...
  SetImageLayerPairList(aContext, *aRuleData->ValueForBackgroundRepeat(),
                        bg->mImage.mLayers, parentBG->mImage.mLayers,
                        &nsStyleImageLayers::Layer::mRepeat, initialRepeat,
                        parentBG->mImage.mRepeatCount, bg->mImage.mRepeatCount,
                        maxItemCount, rebuild, conditions);

  // background-attachment: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForBackgroundAttachment(), bg->mImage.mLayers,
      parentBG->mImage.mLayers, &nsStyleImageLayers::Layer::mAttachment,
      uint8_t(NS_STYLE_IMAGELAYER_ATTACHMENT_SCROLL),
      parentBG->mImage.mAttachmentCount, bg->mImage.mAttachmentCount,
      maxItemCount, rebuild, conditions);
  ...
  SetImageLayerList(aContext, *aRuleData->ValueForBackgroundClip(),
                    ...
                    StyleGeometryBox::BorderBox, parentBG->mImage.mClipCount,
                    bg->mImage.mClipCount, maxItemCount, rebuild, conditions);

  // background-blend-mode: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForBackgroundBlendMode(), bg->mImage.mLayers,
      parentBG->mImage.mLayers, &nsStyleImageLayers::Layer::mBlendMode,
      uint8_t(NS_STYLE_BLEND_NORMAL), parentBG->mImage.mBlendModeCount,
      bg->mImage.mBlendModeCount, maxItemCount, rebuild, conditions);
  ...
  SetImageLayerList(aContext, *aRuleData->ValueForBackgroundOrigin(),
                    ...
                    StyleGeometryBox::PaddingBox, parentBG->mImage.mOriginCount,
                    bg->mImage.mOriginCount, maxItemCount, rebuild, conditions);
  ...
  SetImageLayerPositionCoordList(
      aContext, *aRuleData->ValueForBackgroundPositionX(), bg->mImage.mLayers,
      parentBG->mImage.mLayers, &Position::mXPosition, initialPositionCoord,
      parentBG->mImage.mPositionXCount, bg->mImage.mPositionXCount,
      maxItemCount, rebuild, conditions);
  SetImageLayerPositionCoordList(
      aContext, *aRuleData->ValueForBackgroundPositionY(), bg->mImage.mLayers,
      parentBG->mImage.mLayers, &Position::mYPosition, initialPositionCoord,
      parentBG->mImage.mPositionYCount, bg->mImage.mPositionYCount,
      maxItemCount, rebuild, conditions);
  ...
  SetImageLayerPairList(aContext, *aRuleData->ValueForBackgroundSize(),
                        bg->mImage.mLayers, parentBG->mImage.mLayers,
                        &nsStyleImageLayers::Layer::mSize, initialSize,
                        parentBG->mImage.mSizeCount, bg->mImage.mSizeCount,
                        maxItemCount, rebuild, conditions);
...
const void* nsRuleNode::ComputeMarginData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  NS_FOR_CSS_SIDES(side) {
    ...
    if (SetCoord(*aRuleData->ValueFor(subprops[side]), coord, parentCoord,
...
static void SetBorderImageRect(const nsCSSValue& aValue,
...
static void SetBorderImagePair(const nsCSSValue& aValue,
...
static void SetBorderImageSlice(const nsCSSValue& aValue,
...
const void* nsRuleNode::ComputeBorderData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForBoxDecorationBreak(),
           ...
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentBorder->mBoxDecorationBreak, StyleBoxDecorationBreak::Slice);
  ...
  nsStyleCoord coord;
  {
    ...
    NS_FOR_CSS_SIDES(side) {
      ...
      Maybe<nscoord> coord = ComputeLineWidthValue<eUnsetInitial>(
  ...
  // border-style, border-*-style: enum, inherit
  {
    ...
    NS_FOR_CSS_SIDES(side) {
      ...
      } else if (eCSSUnit_Initial == unit || eCSSUnit_Unset == unit) {
  ...
  // border-color, border-*-color: color, string, enum, inherit
  {
    ...
    NS_FOR_CSS_SIDES(side) {
      SetComplexColor<eUnsetInitial>(
          *aRuleData->ValueFor(subprops[side]),
          parentBorder->mBorderColor[side], StyleComplexColor::CurrentColor(),
          mPresContext, border->mBorderColor[side], conditions);
    }
  }
  ...
  SetValue(*aRuleData->ValueForFloatEdge(), border->mFloatEdge, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentBorder->mFloatEdge,
  ...
  if (borderImageSource->GetUnit() == eCSSUnit_Inherit) {
    ...
    SetStyleImage(aContext, *borderImageSource, border->mBorderImageSource,
                  conditions);
  }
  ...
  SetValue(borderImageSliceFill, border->mBorderImageFill, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentBorder->mBorderImageFill, NS_STYLE_BORDER_IMAGE_SLICE_NOFILL);

  nsCSSRect borderImageSlice;
  SetBorderImageRect(borderImageSliceValue, borderImageSlice);

  nsCSSRect borderImageWidth;
  SetBorderImageRect(*aRuleData->ValueForBorderImageWidth(), borderImageWidth);
  ...
  NS_FOR_CSS_SIDES(side) {
    ...
    if (SetCoord(borderImageSlice.*(nsCSSRect::sides[side]), coord,
                 ...
                 SETCOORD_FACTOR | SETCOORD_PERCENT | SETCOORD_INHERIT |
                     SETCOORD_INITIAL_HUNDRED_PCT | SETCOORD_UNSET_INITIAL,
                 aContext, mPresContext, conditions)) {
    ...
    if (SetCoord(borderImageOutset.*(nsCSSRect::sides[side]), coord,
                 ...
                 SETCOORD_LENGTH | SETCOORD_FACTOR | SETCOORD_INHERIT |
                     SETCOORD_INITIAL_FACTOR_ZERO | SETCOORD_UNSET_INITIAL,
                 aContext, mPresContext, conditions)) {
  ...
  SetValue(borderImageRepeat.mXValue, border->mBorderImageRepeatH, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentBorder->mBorderImageRepeatH, StyleBorderImageRepeat::Stretch);

  SetValue(borderImageRepeat.mYValue, border->mBorderImageRepeatV, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentBorder->mBorderImageRepeatV, StyleBorderImageRepeat::Stretch);

  COMPUTE_END_RESET(Border, border)
}

const void* nsRuleNode::ComputePaddingData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
...
const void* nsRuleNode::ComputeOutlineData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetComplexColor<eUnsetInitial>(
      *aRuleData->ValueForOutlineColor(), parentOutline->mOutlineColor,
      StyleComplexColor::CurrentColor(), mPresContext, outline->mOutlineColor,
      conditions);
...
const void* nsRuleNode::ComputeListData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  auto setListStyleType = [this, list](nsAtom* type) {
    list->mCounterStyle =
        mPresContext->CounterStyleManager()->BuildCounterStyle(type);
  };
  ...
  SetValue(*aRuleData->ValueForListStylePosition(), list->mListStylePosition,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
    ...
    case eCSSUnit_Rect: {
      ...
      if (rgnRect.mBottom.GetUnit() == eCSSUnit_Auto)
        ...
        list->mImageRegion.height =
            CalcLength(rgnRect.mBottom, aContext, mPresContext, conditions) -
            list->mImageRegion.y;
      ...
      if (rgnRect.mRight.GetUnit() == eCSSUnit_Auto)
        ...
        list->mImageRegion.width =
            CalcLength(rgnRect.mRight, aContext, mPresContext, conditions) -
            list->mImageRegion.x;
      break;
    }
...
static void SetGridTrackBreadth(const nsCSSValue& aValue, nsStyleCoord& aResult,
                                GeckoStyleContext* aStyleContext,
                                nsPresContext* aPresContext,
                                RuleNodeCacheConditions& aConditions) {
  nsCSSUnit unit = aValue.GetUnit();
  if (unit == eCSSUnit_FlexFraction) {
    ...
    const nsStyleCoord dummyParentCoord;
    DebugOnly<bool> stored = SetCoord(aValue, aResult, dummyParentCoord,
...
static void SetGridTrackSize(const nsCSSValue& aValue, nsStyleCoord& aResultMin,
  ...
  if (aValue.GetUnit() == eCSSUnit_Function) {
    ...
    if (funcName == eCSSKeyword_minmax) {
      SetGridTrackBreadth(func->Item(1), aResultMin, aStyleContext,
                          aPresContext, aConditions);
      SetGridTrackBreadth(func->Item(2), aResultMax, aStyleContext,
                          aPresContext, aConditions);
    } else if (funcName == eCSSKeyword_fit_content) {
      // We represent fit-content(L) as 'none' min-sizing and L max-sizing.
      SetGridTrackBreadth(nsCSSValue(eCSSUnit_None), aResultMin, aStyleContext,
                          aPresContext, aConditions);
      SetGridTrackBreadth(func->Item(1), aResultMax, aStyleContext,
                          aPresContext, aConditions);
    } else {
    ...
    SetGridTrackBreadth(aValue, aResultMin, aStyleContext, aPresContext,
                        aConditions);
    aResultMax = aResultMin;
  }
}

static void SetGridAutoColumnsRows(
    const nsCSSValue& aValue, nsStyleCoord& aResultMin,
    nsStyleCoord& aResultMax, const nsStyleCoord& aParentValueMin,
    const nsStyleCoord& aParentValueMax, GeckoStyleContext* aStyleContext,
    nsPresContext* aPresContext, RuleNodeCacheConditions& aConditions)

{
  switch (aValue.GetUnit()) {
    ...
    default:
      SetGridTrackSize(aValue, aResultMin, aResultMax, aStyleContext,
                       aPresContext, aConditions);
  }
}

static void AppendGridLineNames(const nsCSSValue& aValue,
...
static void SetGridTrackList(const nsCSSValue& aValue,
      ...
      if (item->mValue.GetUnit() == eCSSUnit_Enumerated &&
        ...
        for (int32_t i = 0; item && i < nsStyleGridLine::kMaxLine; ++i) {
          if (item->mValue.GetUnit() == eCSSUnit_Pair) {
            ...
            MOZ_ASSERT(
                pair.mXValue.GetIntValue() == NS_STYLE_GRID_REPEAT_AUTO_FILL,
                "unexpected repeat() enum value for subgrid");
            const nsCSSValueList* list = pair.mYValue.GetListValue();
            AppendGridLineNames(list->mValue,
                                aResult->mRepeatAutoLineNameListBefore);
          } else {
        ...
        for (int32_t line = 1;; ++line) {
          ...
          if (item->mValue.GetUnit() == eCSSUnit_Pair) {
            ...
            list = list->mNext;
            nsStyleCoord& min =
                *aResult->mMinTrackSizingFunctions.AppendElement();
            nsStyleCoord& max =
                *aResult->mMaxTrackSizingFunctions.AppendElement();
            SetGridTrackSize(list->mValue, min, max, aStyleContext,
                             aPresContext, aConditions);
            list = list->mNext;
            AppendGridLineNames(list->mValue,
                                aResult->mRepeatAutoLineNameListAfter);
          } else {
            nsStyleCoord& min =
                *aResult->mMinTrackSizingFunctions.AppendElement();
            nsStyleCoord& max =
                *aResult->mMaxTrackSizingFunctions.AppendElement();
            SetGridTrackSize(item->mValue, min, max, aStyleContext,
                             aPresContext, aConditions);
          }
...
static void SetGridTemplateAreas(const nsCSSValue& aValue,
...
static void SetGridLine(const nsCSSValue& aValue, nsStyleGridLine& aResult,
    ...
    default:
      ...
      do {
        if (item->mValue.GetUnit() == eCSSUnit_Enumerated) {
          ...
          aResult.mInteger =
              clamped(item->mValue.GetIntValue(), nsStyleGridLine::kMinLine,
...
const void* nsRuleNode::ComputePositionData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  NS_FOR_CSS_SIDES(side) {
    ...
    if (SetCoord(*aRuleData->ValueFor(offsetProps[side]), coord, parentCoord,
  ...
  SetValue(*aRuleData->ValueForBoxSizing(), pos->mBoxSizing, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mBoxSizing,
           StyleBoxSizing::Content);

  // align-content: enum, inherit, initial
  SetValue(*aRuleData->ValueForAlignContent(), pos->mAlignContent, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mAlignContent,
           NS_STYLE_ALIGN_NORMAL);

  // align-items: enum, inherit, initial
  SetValue(*aRuleData->ValueForAlignItems(), pos->mAlignItems, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mAlignItems,
           NS_STYLE_ALIGN_NORMAL);

  // align-self: enum, inherit, initial
  SetValue(*aRuleData->ValueForAlignSelf(), pos->mAlignSelf, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mAlignSelf,
           NS_STYLE_ALIGN_AUTO);

  // justify-content: enum, inherit, initial
  SetValue(*aRuleData->ValueForJustifyContent(), pos->mJustifyContent,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentPos->mJustifyContent, NS_STYLE_JUSTIFY_NORMAL);
  ...
  if (MOZ_UNLIKELY(justifyItemsValue.GetUnit() == eCSSUnit_Inherit)) {
    pos->mSpecifiedJustifyItems = MOZ_LIKELY(parentContext)
                                      ? parentPos->mJustifyItems
                                      : NS_STYLE_JUSTIFY_NORMAL;
    conditions.SetUncacheable();
  } else {
    SetValue(
        justifyItemsValue, pos->mSpecifiedJustifyItems, conditions,
        SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
  ...
  pos->mJustifyItems = pos->mSpecifiedJustifyItems == NS_STYLE_JUSTIFY_AUTO
                           ? NS_STYLE_JUSTIFY_NORMAL
                           : pos->mSpecifiedJustifyItems;

  // justify-self: enum, inherit, initial
  SetValue(*aRuleData->ValueForJustifySelf(), pos->mJustifySelf, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mJustifySelf,
           NS_STYLE_JUSTIFY_AUTO);
  ...
  SetCoord(*aRuleData->ValueForFlexBasis(), pos->mFlexBasis,
           parentPos->mFlexBasis,
           SETCOORD_LPAEH | SETCOORD_INITIAL_AUTO | SETCOORD_STORE_CALC |
               SETCOORD_UNSET_INITIAL,
           aContext, mPresContext, conditions);

  // flex-direction: enum, inherit, initial
  SetValue(*aRuleData->ValueForFlexDirection(), pos->mFlexDirection, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mFlexDirection,
           NS_STYLE_FLEX_DIRECTION_ROW);

  // flex-grow: float, inherit, initial
  SetFactor(*aRuleData->ValueForFlexGrow(), pos->mFlexGrow, conditions,
            parentPos->mFlexGrow, 0.0f, SETFCT_UNSET_INITIAL);

  // flex-shrink: float, inherit, initial
  SetFactor(*aRuleData->ValueForFlexShrink(), pos->mFlexShrink, conditions,
            parentPos->mFlexShrink, 1.0f, SETFCT_UNSET_INITIAL);

  // flex-wrap: enum, inherit, initial
  SetValue(*aRuleData->ValueForFlexWrap(), pos->mFlexWrap, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mFlexWrap,
           NS_STYLE_FLEX_WRAP_NOWRAP);

  // order: integer, inherit, initial
  SetValue(*aRuleData->ValueForOrder(), pos->mOrder, conditions,
           SETVAL_INTEGER | SETVAL_UNSET_INITIAL, parentPos->mOrder,
           NS_STYLE_ORDER_INITIAL);

  // object-fit: enum, inherit, initial
  SetValue(*aRuleData->ValueForObjectFit(), pos->mObjectFit, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentPos->mObjectFit,
  ...
  switch (objectPosition.GetUnit()) {
    ...
    case eCSSUnit_Unset:
      ...
      break;
    default:
      ComputePositionValue(aContext, objectPosition, pos->mObjectPosition,
                           conditions);
  }
  ...
  SetGridAutoColumnsRows(
      *aRuleData->ValueForGridAutoColumns(), pos->mGridAutoColumnsMin,
      pos->mGridAutoColumnsMax, parentPos->mGridAutoColumnsMin,
      parentPos->mGridAutoColumnsMax, aContext, mPresContext, conditions);

  // grid-auto-rows
  SetGridAutoColumnsRows(
      *aRuleData->ValueForGridAutoRows(), pos->mGridAutoRowsMin,
      pos->mGridAutoRowsMax, parentPos->mGridAutoRowsMin,
      parentPos->mGridAutoRowsMax, aContext, mPresContext, conditions);

  // grid-template-columns
  SetGridTrackList(*aRuleData->ValueForGridTemplateColumns(),
                   pos->mGridTemplateColumns,
                   parentPos->mGridTemplateColumns.get(), aContext,
                   mPresContext, conditions);

  // grid-template-rows
  SetGridTrackList(*aRuleData->ValueForGridTemplateRows(),
                   pos->mGridTemplateRows, parentPos->mGridTemplateRows.get(),
                   aContext, mPresContext, conditions);
  ...
  // grid-column-start
  SetGridLine(*aRuleData->ValueForGridColumnStart(), pos->mGridColumnStart,
              parentPos->mGridColumnStart, conditions);

  // grid-column-end
  SetGridLine(*aRuleData->ValueForGridColumnEnd(), pos->mGridColumnEnd,
              parentPos->mGridColumnEnd, conditions);

  // grid-row-start
  SetGridLine(*aRuleData->ValueForGridRowStart(), pos->mGridRowStart,
              parentPos->mGridRowStart, conditions);

  // grid-row-end
  SetGridLine(*aRuleData->ValueForGridRowEnd(), pos->mGridRowEnd,
              parentPos->mGridRowEnd, conditions);

  // grid-column-gap
  if (SetCoord(*aRuleData->ValueForGridColumnGap(), pos->mGridColumnGap,
               parentPos->mGridColumnGap,
               SETCOORD_LPH | SETCOORD_INITIAL_ZERO | SETCOORD_STORE_CALC |
  ...
  // grid-row-gap
  if (SetCoord(*aRuleData->ValueForGridRowGap(), pos->mGridRowGap,
               parentPos->mGridRowGap,
               SETCOORD_LPH | SETCOORD_INITIAL_ZERO | SETCOORD_STORE_CALC |
                   SETCOORD_CALC_CLAMP_NONNEGATIVE | SETCOORD_UNSET_INITIAL,
               aContext, mPresContext, conditions)) {
  } else {
...
const void* nsRuleNode::ComputeTableData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForTableLayout(), table->mLayoutStrategy,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentTable->mLayoutStrategy, NS_STYLE_TABLE_LAYOUT_AUTO);
...
const void* nsRuleNode::ComputeTableBorderData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForBorderCollapse(), table->mBorderCollapse,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentTable->mBorderCollapse, NS_STYLE_BORDER_SEPARATE);
...
#endif
        SetPairCoords(
            *borderSpacingValue, coordCol, coordRow, parentCol, parentRow,
            SETCOORD_LH | SETCOORD_INITIAL_ZERO | SETCOORD_CALC_LENGTH_ONLY |
  ...
  SetValue(*aRuleData->ValueForCaptionSide(), table->mCaptionSide, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentTable->mCaptionSide,
           NS_STYLE_CAPTION_SIDE_TOP);

  // empty-cells: enum, inherit, initial
  SetValue(*aRuleData->ValueForEmptyCells(), table->mEmptyCells, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentTable->mEmptyCells,
           NS_STYLE_TABLE_EMPTY_CELLS_SHOW);

  COMPUTE_END_INHERITED(TableBorder, table)
}

const void* nsRuleNode::ComputeContentData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  uint32_t count;
  ...
  switch (contentValue->GetUnit()) {
    ...
    case eCSSUnit_Enumerated: {
      MOZ_ASSERT(
          contentValue->GetIntValue() == int32_t(StyleContent::AltContent),
    ...
    case eCSSUnit_ListDep: {
      ...
      while (contentValueList) {
        ...
        switch (unit) {
          ...
          case eCSSUnit_String:
          case eCSSUnit_Attr: {
            nsStyleContentType type = unit == eCSSUnit_String
          ...
          case eCSSUnit_Counters: {
            nsStyleContentType type = unit == eCSSUnit_Counter
            ...
            if (style.GetUnit() == eCSSUnit_AtomIdent) {
              counterFunc->mCounterStyle =
                  mPresContext->CounterStyleManager()->BuildCounterStyle(
                      style.GetAtomValue());
  ...
  switch (counterIncrementValue->GetUnit()) {
    ...
    case eCSSUnit_Inherit:
      ...
      while (count--) {
        const nsStyleCounterData& data =
            parentContent->CounterIncrementAt(count);
    ...
    case eCSSUnit_PairListDep: {
      ...
      count = 0;
      for (const nsCSSValuePairList *p = ourIncrement; p;
           p = p->mNext, count++) {
...
const void* nsRuleNode::ComputeXULData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForBoxAlign(), xul->mBoxAlign, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentXUL->mBoxAlign,
           StyleBoxAlign::Stretch);

  // box-direction: enum, inherit, initial
  SetValue(*aRuleData->ValueForBoxDirection(), xul->mBoxDirection, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentXUL->mBoxDirection,
           StyleBoxDirection::Normal);

  // box-flex: factor, inherit
  SetFactor(*aRuleData->ValueForBoxFlex(), xul->mBoxFlex, conditions,
            parentXUL->mBoxFlex, 0.0f, SETFCT_UNSET_INITIAL);

  // box-orient: enum, inherit, initial
  SetValue(*aRuleData->ValueForBoxOrient(), xul->mBoxOrient, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentXUL->mBoxOrient,
           StyleBoxOrient::Horizontal);

  // box-pack: enum, inherit, initial
  SetValue(*aRuleData->ValueForBoxPack(), xul->mBoxPack, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentXUL->mBoxPack,
           StyleBoxPack::Start);

  // box-ordinal-group: integer, inherit, initial
  SetValue(*aRuleData->ValueForBoxOrdinalGroup(), xul->mBoxOrdinal, conditions,
           SETVAL_INTEGER | SETVAL_UNSET_INITIAL, parentXUL->mBoxOrdinal, 1);

  SetValue(*aRuleData->ValueForStackSizing(), xul->mStackSizing, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentXUL->mStackSizing,
           StyleStackSizing::StretchToFit);

  COMPUTE_END_RESET(XUL, xul)
}

const void* nsRuleNode::ComputeColumnData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetCoord(*aRuleData->ValueForColumnWidth(), column->mColumnWidth,
           parent->mColumnWidth,
  ...
  SetCoord(*aRuleData->ValueForColumnGap(), column->mColumnGap,
           parent->mColumnGap,
           SETCOORD_LH | SETCOORD_NORMAL | SETCOORD_INITIAL_NORMAL |
  ...
  if (eCSSUnit_Auto == columnCountValue->GetUnit() ||
    ...
    column->mColumnCount =
        std::min(column->mColumnCount, nsStyleColumn::kMaxColumnCount);
  ...
  SetComplexColor<eUnsetInitial>(
      *aRuleData->ValueForColumnRuleColor(), parent->mColumnRuleColor,
      StyleComplexColor::CurrentColor(), mPresContext, column->mColumnRuleColor,
      conditions);

  // column-fill: enum
  SetValue(*aRuleData->ValueForColumnFill(), column->mColumnFill, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parent->mColumnFill,
           NS_STYLE_COLUMN_FILL_BALANCE);

  // column-span: enum
  SetValue(*aRuleData->ValueForColumnSpan(), column->mColumnSpan, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parent->mColumnSpan,
           NS_STYLE_COLUMN_SPAN_NONE);

  COMPUTE_END_RESET(Column, column)
}

static void SetSVGPaint(const nsCSSValue& aValue,
                        const nsStyleSVGPaint& parentPaint,
                        nsPresContext* aPresContext,
                        GeckoStyleContext* aContext, nsStyleSVGPaint& aResult,
  ...
  if (aValue.GetUnit() == eCSSUnit_Inherit ||
  ...
  } else if (SetColor(aValue, NS_RGB(0, 0, 0), aPresContext, aContext, color,
                      aConditions)) {
    ...
    if (pair.mYValue.GetUnit() == eCSSUnit_None) {
      ...
      SetColor(pair.mYValue, NS_RGB(0, 0, 0), aPresContext, aContext, fallback,
               aConditions);
    }

    if (pair.mXValue.GetUnit() == eCSSUnit_URL) {
      aResult.SetPaintServer(pair.mXValue.GetURLStructValue(), fallbackType,
                             fallback);
    } else if (pair.mXValue.GetUnit() == eCSSUnit_Enumerated) {
      switch (pair.mXValue.GetIntValue()) {
        case NS_COLOR_CONTEXT_FILL:
          aResult.SetContextValue(eStyleSVGPaintType_ContextFill, fallbackType,
                                  fallback);
          break;
        case NS_COLOR_CONTEXT_STROKE:
          aResult.SetContextValue(eStyleSVGPaintType_ContextStroke,
...
static void SetSVGOpacity(const nsCSSValue& aValue, float& aOpacityField,
                          nsStyleSVGOpacitySource& aOpacityTypeField,
                          RuleNodeCacheConditions& aConditions,
                          float aParentOpacity,
  ...
  if (eCSSUnit_Enumerated == aValue.GetUnit()) {
    ...
    SetFactor(aValue, aOpacityField, aConditions, aParentOpacity, 1.0f,
              SETFCT_OPACITY);
    aOpacityTypeField = eStyleSVGOpacitySource_Normal;
  }
}

const void* nsRuleNode::ComputeSVGData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  SetValue(*aRuleData->ValueForClipRule(), svg->mClipRule, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentSVG->mClipRule,
           StyleFillRule::Nonzero);

  // color-interpolation: enum, inherit, initial
  SetValue(*aRuleData->ValueForColorInterpolation(), svg->mColorInterpolation,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentSVG->mColorInterpolation, NS_STYLE_COLOR_INTERPOLATION_SRGB);
  ...
  // fill:
  SetSVGPaint(*aRuleData->ValueForFill(), parentSVG->mFill, mPresContext,
              aContext, svg->mFill, eStyleSVGPaintType_Color, conditions);
  ...
  SetSVGOpacity(*aRuleData->ValueForFillOpacity(), svg->mFillOpacity,
                contextFillOpacity, conditions, parentSVG->mFillOpacity,
                parentSVG->FillOpacitySource());
  svg->SetFillOpacitySource(contextFillOpacity);

  // fill-rule: enum, inherit, initial
  SetValue(*aRuleData->ValueForFillRule(), svg->mFillRule, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentSVG->mFillRule,
           StyleFillRule::Nonzero);
  ...
  SetValue(*aRuleData->ValueForShapeRendering(), svg->mShapeRendering,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentSVG->mShapeRendering, NS_STYLE_SHAPE_RENDERING_AUTO);

  // stroke:
  SetSVGPaint(*aRuleData->ValueForStroke(), parentSVG->mStroke, mPresContext,
              aContext, svg->mStroke, eStyleSVGPaintType_None, conditions);
  ...
  switch (strokeDasharrayValue->GetUnit()) {
    ...
    case eCSSUnit_ListDep: {
      ...
      uint32_t i = 0;
      while (nullptr != value) {
        SetCoord(value->mValue, svg->mStrokeDasharray[i++], nsStyleCoord(),
                 SETCOORD_LP | SETCOORD_FACTOR, aContext, mPresContext,
                 conditions);
        value = value->mNext;
      }
      break;
  ...
  svg->SetStrokeDashoffsetFromObject(strokeDashoffsetValue->GetUnit() ==
                                         eCSSUnit_Enumerated &&
                                     strokeDashoffsetValue->GetIntValue() ==
                                         NS_STYLE_STROKE_PROP_CONTEXT_VALUE);
  if (svg->StrokeDashoffsetFromObject()) {
    svg->mStrokeDashoffset.SetCoordValue(0);
  } else {
    SetCoord(*aRuleData->ValueForStrokeDashoffset(), svg->mStrokeDashoffset,
  ...
  SetValue(*aRuleData->ValueForStrokeLinecap(), svg->mStrokeLinecap, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentSVG->mStrokeLinecap,
           NS_STYLE_STROKE_LINECAP_BUTT);

  // stroke-linejoin: enum, inherit, initial
  SetValue(*aRuleData->ValueForStrokeLinejoin(), svg->mStrokeLinejoin,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT,
           parentSVG->mStrokeLinejoin, NS_STYLE_STROKE_LINEJOIN_MITER);

  // stroke-miterlimit: <miterlimit>, inherit
  SetFactor(*aRuleData->ValueForStrokeMiterlimit(), svg->mStrokeMiterlimit,
            conditions, parentSVG->mStrokeMiterlimit, 4.0f,
            SETFCT_UNSET_INHERIT);

  // stroke-opacity:
  nsStyleSVGOpacitySource contextStrokeOpacity = svg->StrokeOpacitySource();
  SetSVGOpacity(*aRuleData->ValueForStrokeOpacity(), svg->mStrokeOpacity,
                contextStrokeOpacity, conditions, parentSVG->mStrokeOpacity,
                parentSVG->StrokeOpacitySource());
  ...
  switch (strokeWidthValue->GetUnit()) {
    case eCSSUnit_Enumerated:
      MOZ_ASSERT(
          strokeWidthValue->GetIntValue() == NS_STYLE_STROKE_PROP_CONTEXT_VALUE,
    ...
    default:
      svg->SetStrokeWidthFromObject(false);
      SetCoord(*strokeWidthValue, svg->mStrokeWidth, parentSVG->mStrokeWidth,
               SETCOORD_LPH | SETCOORD_FACTOR | SETCOORD_UNSET_INHERIT,
               aContext, mPresContext, conditions);
  }

  // text-anchor: enum, inherit, initial
  SetValue(*aRuleData->ValueForTextAnchor(), svg->mTextAnchor, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INHERIT, parentSVG->mTextAnchor,
  ...
  switch (contextPropsValue->GetUnit()) {
    ...
    case eCSSUnit_ListDep: {
      ...
      for (const nsCSSValueList* item = contextPropsValue->GetListValue(); item;
           item = item->mNext) {
        nsAtom* atom = item->mValue.GetAtomValue();
...
static UniquePtr<StyleBasicShape> GetStyleBasicShapeFromCSSValue(
    const nsCSSValue& aValue, GeckoStyleContext* aStyleContext,
    nsPresContext* aPresContext, RuleNodeCacheConditions& aConditions) {
  ...
  if (functionName == eCSSKeyword_polygon) {
    ...
    const int32_t mask =
        SETCOORD_PERCENT | SETCOORD_LENGTH | SETCOORD_STORE_CALC;
    ...
    while (curPair) {
      ...
      DebugOnly<bool> didSetCoordX =
          SetCoord(curPair->mXValue, xCoord, nsStyleCoord(), mask,
                   aStyleContext, aPresContext, aConditions);
      coordinates.AppendElement(xCoord);
      MOZ_ASSERT(didSetCoordX, "unexpected x coordinate unit");
      DebugOnly<bool> didSetCoordY =
          SetCoord(curPair->mYValue, yCoord, nsStyleCoord(), mask,
                   aStyleContext, aPresContext, aConditions);
    ...
    for (size_t j = 1; j < count; ++j) {
      ...
      if (val.GetUnit() != eCSSUnit_Null) {
        DebugOnly<bool> didSetRadius =
            SetCoord(val, radius, nsStyleCoord(), mask, aStyleContext,
                     aPresContext, aConditions);
    ...
    if (positionVal.GetUnit() == eCSSUnit_Array) {
      ComputePositionValue(aStyleContext, positionVal,
                           basicShape->GetPosition(), aConditions);
    } else {
      MOZ_ASSERT(positionVal.GetUnit() == eCSSUnit_Null, "expected no value");
    }
    ...
    if (shapeFunction->Item(5).GetUnit() == eCSSUnit_Array) {
      ...
      NS_FOR_CSS_FULL_CORNERS(corner) {
        ...
        nsStyleCoord coordX, coordY;
        DebugOnly<bool> didSetRadii = SetPairCoords(
            radius, coordX, coordY, nsStyleCoord(), nsStyleCoord(), mask,
            aStyleContext, aPresContext, aConditions);
      ...
      NS_FOR_CSS_HALF_CORNERS(j) { insetRadius.Set(j, zero); }
    }
  } else {
...
static void SetStyleShapeSourceToCSSValue(
    StyleShapeSource* aShapeSource, const nsCSSValue* aValue,
    GeckoStyleContext* aStyleContext, nsPresContext* aPresContext,
    RuleNodeCacheConditions& aConditions) {
...
static bool SetStyleFilterToCSSValue(nsStyleFilter* aStyleFilter,
                                     const nsCSSValue& aValue,
                                     GeckoStyleContext* aStyleContext,
                                     nsPresContext* aPresContext,
                                     RuleNodeCacheConditions& aConditions) {
  ...
  int32_t type;
  DebugOnly<bool> foundKeyword = nsCSSProps::FindKeyword(
      functionName, nsCSSProps::kFilterFunctionKTable, type);
  MOZ_ASSERT(foundKeyword, "unknown filter type");
  if (type == NS_STYLE_FILTER_DROP_SHADOW) {
    RefPtr<nsCSSShadowArray> shadowArray =
        GetShadowData(filterFunction->Item(1).GetListValue(), aStyleContext,
                      false, aPresContext, aConditions);
  ...
  if (type == NS_STYLE_FILTER_BLUR) {
    mask = SETCOORD_LENGTH | SETCOORD_CALC_LENGTH_ONLY |
           SETCOORD_CALC_CLAMP_NONNEGATIVE;
...
const void* nsRuleNode::ComputeSVGResetData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  if (eCSSUnit_Initial == stopColorValue->GetUnit() ||
    ...
    SetColor(*stopColorValue, parentSVGReset->mStopColor, mPresContext,
             aContext, svgReset->mStopColor, conditions);
  }
  ...
  if (eCSSUnit_Initial == floodColorValue->GetUnit() ||
    ...
    SetColor(*floodColorValue, parentSVGReset->mFloodColor, mPresContext,
             aContext, svgReset->mFloodColor, conditions);
  }
  ...
  if (eCSSUnit_Initial == lightingColorValue->GetUnit() ||
    ...
    SetColor(*lightingColorValue, parentSVGReset->mLightingColor, mPresContext,
             aContext, svgReset->mLightingColor, conditions);
  }
  ...
  switch (clipPathValue->GetUnit()) {
    ...
    case eCSSUnit_Array: {
      svgReset->mClipPath = StyleShapeSource();
      SetStyleShapeSourceToCSSValue(&svgReset->mClipPath, clipPathValue,
                                    aContext, mPresContext, conditions);
      break;
    }
    default:
  ...
  // stop-opacity:
  SetFactor(*aRuleData->ValueForStopOpacity(), svgReset->mStopOpacity,
            conditions, parentSVGReset->mStopOpacity, 1.0f,
            SETFCT_OPACITY | SETFCT_UNSET_INITIAL);

  // flood-opacity:
  SetFactor(*aRuleData->ValueForFloodOpacity(), svgReset->mFloodOpacity,
            conditions, parentSVGReset->mFloodOpacity, 1.0f,
            SETFCT_OPACITY | SETFCT_UNSET_INITIAL);

  // dominant-baseline: enum, inherit, initial
  SetValue(*aRuleData->ValueForDominantBaseline(), svgReset->mDominantBaseline,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentSVGReset->mDominantBaseline, NS_STYLE_DOMINANT_BASELINE_AUTO);

  // vector-effect: enum, inherit, initial
  SetValue(*aRuleData->ValueForVectorEffect(), svgReset->mVectorEffect,
           conditions, SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL,
           parentSVGReset->mVectorEffect, NS_STYLE_VECTOR_EFFECT_NONE);

  // mask-type: enum, inherit, initial
  SetValue(*aRuleData->ValueForMaskType(), svgReset->mMaskType, conditions,
           SETVAL_ENUMERATED | SETVAL_UNSET_INITIAL, parentSVGReset->mMaskType,
  ...
  nsStyleImage initialImage;
  SetImageLayerList(
      aContext, *aRuleData->ValueForMaskImage(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mImage,
      initialImage, parentSVGReset->mMask.mImageCount,
      svgReset->mMask.mImageCount, maxItemCount, rebuild, conditions);

  // mask-repeat: enum, inherit, initial [pair list]
  nsStyleImageLayers::Repeat initialRepeat;
  initialRepeat.SetInitialValues();
  SetImageLayerPairList(
      aContext, *aRuleData->ValueForMaskRepeat(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mRepeat,
      initialRepeat, parentSVGReset->mMask.mRepeatCount,
      svgReset->mMask.mRepeatCount, maxItemCount, rebuild, conditions);

  // mask-clip: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForMaskClip(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mClip,
      StyleGeometryBox::BorderBox, parentSVGReset->mMask.mClipCount,
      svgReset->mMask.mClipCount, maxItemCount, rebuild, conditions);

  // mask-origin: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForMaskOrigin(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mOrigin,
      StyleGeometryBox::BorderBox, parentSVGReset->mMask.mOriginCount,
      svgReset->mMask.mOriginCount, maxItemCount, rebuild, conditions);
  ...
  SetImageLayerPositionCoordList(
      aContext, *aRuleData->ValueForMaskPositionX(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &Position::mXPosition,
      initialPositionCoord, parentSVGReset->mMask.mPositionXCount,
      svgReset->mMask.mPositionXCount, maxItemCount, rebuild, conditions);
  SetImageLayerPositionCoordList(
      aContext, *aRuleData->ValueForMaskPositionY(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &Position::mYPosition,
      initialPositionCoord, parentSVGReset->mMask.mPositionYCount,
      svgReset->mMask.mPositionYCount, maxItemCount, rebuild, conditions);

  // mask-size: enum, length, auto, inherit, initial [pair list]
  nsStyleImageLayers::Size initialSize;
  initialSize.SetInitialValues();
  SetImageLayerPairList(
      aContext, *aRuleData->ValueForMaskSize(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mSize,
      initialSize, parentSVGReset->mMask.mSizeCount, svgReset->mMask.mSizeCount,
      maxItemCount, rebuild, conditions);

  // mask-mode: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForMaskMode(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mMaskMode,
      uint8_t(NS_STYLE_MASK_MODE_MATCH_SOURCE),
      parentSVGReset->mMask.mMaskModeCount, svgReset->mMask.mMaskModeCount,
      maxItemCount, rebuild, conditions);

  // mask-composite: enum, inherit, initial [list]
  SetImageLayerList(
      aContext, *aRuleData->ValueForMaskComposite(), svgReset->mMask.mLayers,
      parentSVGReset->mMask.mLayers, &nsStyleImageLayers::Layer::mComposite,
      uint8_t(NS_STYLE_MASK_COMPOSITE_ADD),
      parentSVGReset->mMask.mCompositeCount, svgReset->mMask.mCompositeCount,
      maxItemCount, rebuild, conditions);
...
const void* nsRuleNode::ComputeVariablesData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  resolver.Resolve(&parentVariables->mVariables, aRuleData->mVariables);
  conditions.SetUncacheable();

  COMPUTE_END_INHERITED(Variables, variables)
}

const void* nsRuleNode::ComputeEffectsData(
    void* aStartStruct, const nsRuleData* aRuleData,
    GeckoStyleContext* aContext, nsRuleNode* aHighestNode,
    const RuleDetail aRuleDetail, const RuleNodeCacheConditions aConditions) {
  ...
  switch (boxShadowValue->GetUnit()) {
    ...
    case eCSSUnit_ListDep:
      effects->mBoxShadow =
          GetShadowData(boxShadowValue->GetListValue(), aContext, true,
                        mPresContext, conditions);
      break;
  ...
  switch (clipValue->GetUnit()) {
    ...
    case eCSSUnit_Rect: {
      ...
      if (clipRect.mTop.GetUnit() == eCSSUnit_Auto) {
        ...
        effects->mClip.y =
            CalcLength(clipRect.mTop, aContext, mPresContext, conditions);
      }
      ...
      if (clipRect.mBottom.GetUnit() == eCSSUnit_Auto) {
        ...
        effects->mClip.height =
            CalcLength(clipRect.mBottom, aContext, mPresContext, conditions) -
            effects->mClip.y;
      }
      ...
      if (clipRect.mLeft.GetUnit() == eCSSUnit_Auto) {
        ...
        effects->mClip.x =
            CalcLength(clipRect.mLeft, aContext, mPresContext, conditions);
      }
      ...
      if (clipRect.mRight.GetUnit() == eCSSUnit_Auto) {
        ...
        effects->mClip.width =
            CalcLength(clipRect.mRight, aContext, mPresContext, conditions) -
            effects->mClip.x;
      }
      break;
    }
...
const void* nsRuleNode::GetStyleData(nsStyleStructID aSID,
  ...
  if (MOZ_UNLIKELY(!aComputeData)) return nullptr;
...
void nsRuleNode::GetDiscretelyAnimatedCSSValue(nsCSSPropertyID aProperty,
                                               nsCSSValue* aValue) {
...
/* static */ bool nsRuleNode::HasAuthorSpecifiedRules(
    GeckoStyleContext* aStyleContext, uint32_t ruleTypeMask,
  ...
  if (ruleTypeMask & NS_AUTHOR_SPECIFIED_BACKGROUND) {
    for (uint32_t i = 0, i_end = ArrayLength(backgroundValues); i < i_end;
         ++i) {
  ...
  if (ruleTypeMask & NS_AUTHOR_SPECIFIED_BORDER) {
    for (uint32_t i = 0, i_end = ArrayLength(borderValues); i < i_end; ++i) {
  ...
  if (ruleTypeMask & NS_AUTHOR_SPECIFIED_PADDING) {
    for (uint32_t i = 0, i_end = ArrayLength(paddingValues); i < i_end; ++i) {
  ...
  do {
      ...
      if (rule) {
        ...
        if (ruleData.mLevel == SheetType::Agent ||
          ...
          for (uint32_t i = 0; i < nValues; ++i) {
            ...
            if (unit != eCSSUnit_Null && unit != eCSSUnit_Dummy &&
    ...
    if (haveExplicitUAInherit) {
      ...
      // style context.
      for (uint32_t i = 0; i < nValues; ++i)
        if (values[i]->GetUnit() == eCSSUnit_Null) values[i]->SetDummyValue();
      for (uint32_t i = 0; i < nValues; ++i)
        if (values[i]->GetUnit() == eCSSUnit_DummyInherit) values[i]->Reset();
...
/* static */ void nsRuleNode::ComputePropertiesOverridingAnimation(
  ...
  for (size_t propIdx = 0, propEnd = aProperties.Length(); propIdx < propEnd;
       ++propIdx) {
...
/* static */
bool nsRuleNode::ComputeColor(const nsCSSValue& aValue,
                              nsPresContext* aPresContext,
...
/* static */ bool nsRuleNode::ParentHasPseudoElementData(
    GeckoStyleContext* aContext) {
  GeckoStyleContext* parent = aContext->GetParent();
  return parent && parent->HasPseudoElementData();
}

/* static */ void nsRuleNode::StoreStyleOnContext(GeckoStyleContext* aContext,
...
#ifdef DEBUG
bool nsRuleNode::ContextHasCachedData(GeckoStyleContext* aContext,
                                      nsStyleStructID aSID) {
