...
NS_IMETHODIMP nsStandardURL::GetOriginCharset(nsACString &result) {
...
inline bool IsUTFCharset(const char *aCharset) {
...
nsresult nsStandardURL::Init(uint32_t urlType, int32_t defaultPort,
  ...
  if (!baseURI) return SetSpecInternal(spec);

  nsAutoCString buf;
  nsresult rv = baseURI->Resolve(spec, buf);
  if (NS_FAILED(rv)) return rv;

  return SetSpecInternal(buf);
}
