...
const OperatingSystemVersionInfo kOsVersionStrings[] = {
    ...
    {LookAndFeel::eOperatingSystemVersion_WindowsVista, &nsGkAtoms::windows_win7},
    ...
    {LookAndFeel::eOperatingSystemVersion_Windows10,
     &nsGkAtoms::windows_win10}};
#endif
...
static void GetWidth(nsIDocument* aDocument, const nsMediaFeature*,
                     nsCSSValue& aResult) {
  nsSize size = GetSize(aDocument);
  aResult.SetFloatValue(CSSPixel::FromAppUnits(size.width), eCSSUnit_Pixel);
}

static void GetHeight(nsIDocument* aDocument, const nsMediaFeature*,
                      nsCSSValue& aResult) {
  nsSize size = GetSize(aDocument);
...
static void GetDeviceWidth(nsIDocument* aDocument, const nsMediaFeature*,
                           nsCSSValue& aResult) {
  nsSize size = GetDeviceSize(aDocument);
  aResult.SetFloatValue(CSSPixel::FromAppUnits(size.width), eCSSUnit_Pixel);
}

static void GetDeviceHeight(nsIDocument* aDocument, const nsMediaFeature*,
                            nsCSSValue& aResult) {
  nsSize size = GetDeviceSize(aDocument);
  aResult.SetFloatValue(CSSPixel::FromAppUnits(size.height), eCSSUnit_Pixel);
}

static void GetOrientation(nsIDocument* aDocument, const nsMediaFeature*,
                           nsCSSValue& aResult) {
  nsSize size = GetSize(aDocument);
  // Per spec, square viewports should be 'portrait'
  auto orientation = size.width > size.height ? StyleOrientation::Landscape
                                              : StyleOrientation::Portrait;
  aResult.SetEnumValue(orientation);
}

static void GetDeviceOrientation(nsIDocument* aDocument, const nsMediaFeature*,
                                 nsCSSValue& aResult) {
  nsSize size = GetDeviceSize(aDocument);
  // Per spec, square viewports should be 'portrait'
  auto orientation = size.width > size.height ? StyleOrientation::Landscape
...
static void GetIsResourceDocument(nsIDocument* aDocument, const nsMediaFeature*,
                                  nsCSSValue& aResult) {
  aResult.SetIntValue(aDocument->IsResourceDoc() ? 1 : 0, eCSSUnit_Integer);
}

// Helper for two features below
static void MakeArray(const nsSize& aSize, nsCSSValue& aResult) {
...
static void GetAspectRatio(nsIDocument* aDocument, const nsMediaFeature*,
                           nsCSSValue& aResult) {
  MakeArray(GetSize(aDocument), aResult);
}

static void GetDeviceAspectRatio(nsIDocument* aDocument, const nsMediaFeature*,
                                 nsCSSValue& aResult) {
...
static void GetColorIndex(nsIDocument* aDocument, const nsMediaFeature*,
                          nsCSSValue& aResult) {
...
static void GetMonochrome(nsIDocument* aDocument, const nsMediaFeature*,
                          nsCSSValue& aResult) {
...
static void GetDisplayMode(nsIDocument* aDocument, const nsMediaFeature*,
                           nsCSSValue& aResult) {
...
static void GetGrid(nsIDocument* aDocument, const nsMediaFeature*,
                    nsCSSValue& aResult) {
...
static void GetDevicePixelRatio(nsIDocument* aDocument, const nsMediaFeature*,
                                nsCSSValue& aResult) {
  if (nsContentUtils::ShouldResistFingerprinting(aDocument)) {
...
static void GetSystemMetric(nsIDocument* aDocument,
...
static void GetWindowsTheme(nsIDocument* aDocument,
                            const nsMediaFeature* aFeature,
                            nsCSSValue& aResult) {
  aResult.Reset();
  ...
  if (windowsThemeId == LookAndFeel::eWindowsTheme_Classic) return;
...
static void GetOperatingSystemVersion(nsIDocument* aDocument,
                                      const nsMediaFeature* aFeature,
                                      nsCSSValue& aResult) {
  aResult.Reset();
...
static void GetIsGlyph(nsIDocument* aDocument, const nsMediaFeature* aFeature,
                       nsCSSValue& aResult) {
  MOZ_ASSERT(aFeature->mReqFlags & nsMediaFeature::eUserAgentAndChromeOnly);
  aResult.SetIntValue(aDocument->IsSVGGlyphsDocument() ? 1 : 0,
                      eCSSUnit_Integer);
}
...
/* static */ const nsMediaFeature nsMediaFeatures::features[] = {
    {&nsGkAtoms::width,
