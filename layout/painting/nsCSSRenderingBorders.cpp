...
nsCSSBorderRenderer::nsCSSBorderRenderer(
    ...
    const nsBorderColors* aCompositeColors, bool aBackfaceIsVisible, const Maybe<Rect>& aClipRect)
...
bool nsCSSBorderRenderer::AllBordersSolid(bool *aHasCompositeColors) {
...
void nsCSSBorderRenderer::DrawNoCompositeColorSolidBorder() {
...
void nsCSSBorderRenderer::DrawBorders() {
  ...
  if (allBordersSolid && mAllBordersSameWidth && mCompositeColors[0] == nullptr &&
      mBorderWidths[0] == 1 && mNoBorderRadius && !mAvoidStroke) {
...
bool nsCSSBorderRenderer::CanCreateWebRenderCommands() {
  NS_FOR_CSS_SIDES(i) {
    if (mCompositeColors[i] != nullptr &&
