...
class ConditionVarImpl {
 public:
  ...
  virtual void Wait() = 0;
  virtual void TimedWait(const base::TimeDelta& max_time) = 0;
...
class WinVistaCondVar: public ConditionVarImpl {
 public:
  ...
  void Wait() override;
  void TimedWait(const base::TimeDelta& max_time) override;
...
void WinVistaCondVar::TimedWait(const base::TimeDelta& max_time) {
  DWORD timeout = static_cast<DWORD>(max_time.InMilliseconds());
  CRITICAL_SECTION* cs = reinterpret_cast<PCRITICAL_SECTION>(user_lock_.lock_.native_handle());

  if (FALSE == sleep_condition_variable_fn(reinterpret_cast<PCONDITION_VARIABLE>(&cv_), cs, timeout)) {
...
void WinVistaCondVar::Broadcast() {
  wake_all_condition_variable_fn(reinterpret_cast<PCONDITION_VARIABLE>(&cv_));
}

void WinVistaCondVar::Signal() {
  wake_condition_variable_fn(reinterpret_cast<PCONDITION_VARIABLE>(&cv_));
}
...
void WinXPCondVar::TimedWait(const base::TimeDelta& max_time) {
  Event* waiting_event;
  HANDLE handle;
  {
    AutoLock auto_lock(internal_lock_);
...
void ConditionVariable::Broadcast() { impl_->Broadcast(); }
