...
MP4Demuxer::MP4Demuxer(MediaResource* aResource)
    : mResource(aResource), mStream(new mp4_demuxer::ResourceStream(aResource)) {
}
...
RefPtr<MP4Demuxer::InitPromise> MP4Demuxer::Init() {
  ...
  mp4_demuxer::MP4Metadata::ResultAndByteBuffer initData = mp4_demuxer::MP4Metadata::Metadata(stream);
  if (!initData.Ref()) {
  ...
  RefPtr<mp4_demuxer::BufferStream> bufferstream = new mp4_demuxer::BufferStream(initData.Ref());
  ...
  mp4_demuxer::MP4Metadata::ResultAndCryptoFile cryptoFile = metadata.Crypto();
...
MP4TrackDemuxer::MP4TrackDemuxer(MP4Demuxer* aParent,
    ...
    : mParent(aParent),
      ...
      mInfo(Move(aInfo)),
      mIndex(new mp4_demuxer::Index(aIndices, mStream, mInfo->mTrackId, mInfo->IsAudio())),
...
already_AddRefed<MediaRawData> MP4TrackDemuxer::GetNextSample() {
  ...
  if (mInfo->GetAsVideoInfo()) {
    ...
    if (mIsH264 && !sample->mCrypto.mValid) {
      ...
      switch (type) {
        case mp4_demuxer::H264::FrameType::I_FRAME:
          MOZ_FALLTHROUGH;
...
RefPtr<MP4TrackDemuxer::SamplesPromise> MP4TrackDemuxer::GetSamples(
  ...
  for (const auto& sample : samples->mSamples) {
    ...
    if (mNeedSPSForTelemetry && mIsH264 && mp4_demuxer::AnnexB::IsAVCC(sample)) {
      RefPtr<MediaByteBuffer> extradata = mp4_demuxer::H264::ExtractExtraData(sample);
...
void MP4TrackDemuxer::BreakCycles() { mParent = nullptr; }
