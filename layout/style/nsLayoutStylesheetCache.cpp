...
StyleSheet* nsLayoutStylesheetCache::NumberControlSheet() {
...
StyleSheet* nsLayoutStylesheetCache::XULComponentsSheet() {
  if (!mXULComponentsSheet) {
    LoadSheetURL("chrome://global/content/components.css", &mXULComponentsSheet,
                 eAgentSheetFeatures, eCrash);
  }
...
nsLayoutStylesheetCache::nsLayoutStylesheetCache(StyleBackendType aType)
  ...
  if (userContentSheetURL) {
    MOZ_ASSERT(XRE_IsContentProcess(), "Only used in content processes.");
    LoadSheet(userContentSheetURL, &mUserContentSheet, eUserSheetFeatures,
              eLogToConsole);
...
/* static */ nsLayoutStylesheetCache* nsLayoutStylesheetCache::For(
    StyleBackendType aType) {
  MOZ_ASSERT(NS_IsMainThread());

  bool mustInit = !gStyleCache_Gecko && !gStyleCache_Servo;
  auto& cache =
      aType == StyleBackendType::Gecko ? gStyleCache_Gecko : gStyleCache_Servo;
...
void nsLayoutStylesheetCache::LoadSheet(nsIURI* aURI,
  ...
  auto& loader = mBackendType == StyleBackendType::Gecko ? gCSSLoader_Gecko
                                                         : gCSSLoader_Servo;
...
/* static */ void nsLayoutStylesheetCache::InvalidateSheet(
    RefPtr<StyleSheet>* aGeckoSheet, RefPtr<StyleSheet>* aServoSheet) {
...
/* static */ void nsLayoutStylesheetCache::DependentPrefChanged(
