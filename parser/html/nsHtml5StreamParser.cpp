...
nsresult nsHtml5StreamParser::FinalizeSniffing(
  ...
  if (mCharsetSource == kCharsetUninitialized) {
    ...
    NS_ASSERTION(mCharset.EqualsLiteral("UTF-8"), "XHR should default to UTF-8");
    ...
    switch (mBomState) {
      ...
      case SEEN_UTF_16_LE_FIRST_BYTE:
        if (aFromSegment[i] == 0xFE) {
          rv = SetupDecodingFromBom(
              "UTF-16LE");  // upper case is the raw form
          NS_ENSURE_SUCCESS(rv, rv);
      ...
      case SEEN_UTF_16_BE_FIRST_BYTE:
        if (aFromSegment[i] == 0xFF) {
          rv = SetupDecodingFromBom(
              "UTF-16BE");  // upper case is the raw form
          NS_ENSURE_SUCCESS(rv, rv);
          uint32_t count = aCount - (i + 1);
          rv = WriteStreamBytes(aFromSegment + (i + 1), count, &writeCount);
          NS_ENSURE_SUCCESS(rv, rv);
      ...
      case SEEN_UTF_8_SECOND_BYTE:
        if (aFromSegment[i] == 0xBF) {
          rv = SetupDecodingFromBom(
              "UTF-8");  // upper case is the raw form
          NS_ENSURE_SUCCESS(rv, rv);
          uint32_t count = aCount - (i + 1);
          rv = WriteStreamBytes(aFromSegment + (i + 1), count, &writeCount);
          NS_ENSURE_SUCCESS(rv, rv);
      ...
      if (mFirstBuffer == mLastBuffer) {
        switch (mStreamState) {
          ...
          case STREAM_ENDED:
            ...
            mAtEOF = true;
            if (mCharset.EqualsLiteral("UTF-8") && !mHasHadErrors) {
