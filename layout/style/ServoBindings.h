 ...
 *
 * Functions beginning with Gecko_ are implemented in Gecko and invoked from
 * Servo. Functions beginning with Servo_ are implemented in Servo and invoked
 * from Gecko.
 */
...
void Gecko_ServoStyleContext_Init(
    mozilla::ServoStyleContext* context,
...
mozilla::ServoStyleSheet* Gecko_LoadStyleSheet(
    mozilla::css::Loader* loader, mozilla::ServoStyleSheet* parent,
    mozilla::css::SheetLoadData* parent_load_data,
    mozilla::css::LoaderReusableStyleSheets* reusable_sheets,
    RawGeckoURLExtraData* base_url_data, const uint8_t* url_bytes,
    uint32_t url_length, RawServoMediaListStrong media_list);

// Selector Matching.
uint64_t Gecko_ElementState(RawGeckoElementBorrowed element);
bool Gecko_IsRootElement(RawGeckoElementBorrowed element);
bool Gecko_MatchesElement(mozilla::CSSPseudoClassType type,
                          RawGeckoElementBorrowed element);
...
// nsStyleContent. This is needed to run the destructors, otherwise we'd leak
// the images, strings, and whatnot.
...
void Gecko_CSSValue_SetURL(nsCSSValueBorrowedMut css_value,
                           mozilla::css::URLValue* uri);
...
void Gecko_CSSFontFaceRule_GetCssText(const nsCSSFontFaceRule* rule,
                                      nsAString* result);
...
nsCSSCounterStyleRule* Gecko_CSSCounterStyle_Clone(
    const nsCSSCounterStyleRule* rule);
void Gecko_CSSCounterStyle_GetCssText(const nsCSSCounterStyleRule* rule,
                                      nsAString* result);
...
mozilla::css::ErrorReporter* Gecko_CreateCSSErrorReporter(
    mozilla::ServoStyleSheet* sheet, mozilla::css::Loader* loader, nsIURI* uri);
