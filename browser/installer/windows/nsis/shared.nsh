...
!macro PostUpdate
  ...
  ${AndIf} $9 != 0 ; We're not running in session 0
    ReadRegStr $0 HKCU "Software\Classes\eXPlorerURL\shell\open\command" ""
...
!macro SetHandlers
  ...
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "eXPlorerHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "eXPlorerHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "eXPlorerHTML"
    WriteRegStr SHCTX "$0\.html"  "" "eXPlorerHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "eXPlorerHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "eXPlorerHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "eXPlorerHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "eXPlorerHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "eXPlorerHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "eXPlorerHTML$5"
  ${EndIf}

  ${AddAssociationIfNoneExist} ".pdf" "eXPlorerHTML$5"
  ${AddAssociationIfNoneExist} ".oga" "eXPlorerHTML$5"
  ${AddAssociationIfNoneExist} ".ogg" "eXPlorerHTML$5"
  ${AddAssociationIfNoneExist} ".ogv" "eXPlorerHTML$5"
  ${AddAssociationIfNoneExist} ".pdf" "eXPlorerHTML$5"
  ${AddAssociationIfNoneExist} ".webm" "eXPlorerHTML$5"

  ; An empty string is used for the 5th param because FirefoxHTML is not a
  ; protocol handler
  ${AddDisabledDDEHandlerValues} "eXPlorerHTML$5" "$2" "$8,1" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "eXPlorerURL$5" "$2" "$8,1" "${AppRegName} URL" \
                                 "true"
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "eXPlorerHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "eXPlorerHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "eXPlorerHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "eXPlorerHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "eXPlorerHTML$2"

  WriteRegStr ${RegKey} "$0\Capabilities\StartMenu" "StartMenuInternet" "$1"

  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "ftp"    "eXPlorerURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "eXPlorerURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "eXPlorerURL$2"
...
!ifdef HAVE_64BIT_BUILD
  ...
  ClearErrors
  ReadRegDWORD $2 HKLM "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1"
  ...
  ${AndIf} $2 != 1
    ClearErrors
    WriteRegDWORD HKLM "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1" 1
    ${If} ${Errors}
      ; There was an error writing to HKLM so just write it to HKCU
      WriteRegDWORD HKCU "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1" 1
    ${Else}
      ; This will delete the value from HKCU if it exists
      DeleteRegValue HKCU "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1"
    ${EndIf}
  ${EndIf}

  ClearErrors
  ReadRegDWORD $2 HKCU "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1"
  ; If there were no errors then the system was updated from Firefox 32 bit to
  ; Firefox 64 bit and if the value is already 1 then the registry value has
...
!else
  ...
  ${If} ${RunningX64}
    ...
    ${If} ${Errors}
      ClearErrors
      WriteRegDWORD HKLM "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1" 0
      ; If there were errors write the value in HKCU.
      ${If} ${Errors}
        WriteRegDWORD HKCU "Software\${CompanyName}\${BrandShortName}\32to64DidMigrate" "$1" 0
      ${EndIf}
    ${EndIf}
  ${EndIf}
...
!macro FixShellIconHandler RegKey
  ...
  StrCpy $3 "eXPlorerHTML-$AppUserModelID"
  ClearErrors
  ReadRegStr $0 ${RegKey} "Software\Classes\$3\DefaultIcon" ""
  ${If} ${Errors}
    StrCpy $3 "eXPlorerHTML"
  ${EndIf}
...
!macro SetAppKeys
  ...
  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Main"
  ${WriteRegStr2} $TmpVal "$0" "Install Directory" "$8" 0
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Uninstall"
  ${WriteRegStr2} $TmpVal "$0" "Description" "${BrandFullNameInternal} ${AppVersion}$3 (${ARCH} ${AB_CD})" 0

  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})"
  ...
  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal} ${AppVersion}$3\bin"
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal} ${AppVersion}$3\extensions"
  ${WriteRegStr2} $TmpVal "$0" "Components" "$8\components" 0
  ${WriteRegStr2} $TmpVal "$0" "Plugins" "$8\plugins" 0

  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal} ${AppVersion}$3"
  ...
  StrCpy $0 "Software\${CompanyName}\${BrandFullNameInternal}$3"
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "eXPlorerHTML-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because FirefoxHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "eXPlorerHTML-$AppUserModelID" "$2" "$8,1" \
                                   "${AppRegName} HTML Document" ""
  ${Else}
    ${IsHandlerForInstallDir} "eXPlorerHTML" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "eXPlorerHTML" "$2" "$8,1" \
                                     "${AppRegName} HTML Document" ""
    ${EndIf}
  ${EndIf}

  ${IsHandlerForInstallDir} "eXPlorerURL-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "eXPlorerURL-$AppUserModelID" "$2" "$8,1" \
                                   "${AppRegName} URL" "true"
  ${Else}
    ${IsHandlerForInstallDir} "eXPlorerURL" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "eXPlorerURL" "$2" "$8,1" \
                                     "${AppRegName} URL" "true"
    ${EndIf}
  ${EndIf}
